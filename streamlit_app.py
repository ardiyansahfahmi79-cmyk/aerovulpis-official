...
# ##############################################################################
# AUTHENTICATION CALLBACK (MAGIC LINK + FALLBACK GOOGLE)
# ##############################################################################

def handle_auth_callback():
    """
    Menangani callback dari Magic Link (OTP) atau Google OAuth.
    
    Magic Link:
    - Supabase kirim email dengan tautan ke aplikasi
    - User klik tautan → kembali dengan ?token_hash=...
    - verify_otp() tukar token_hash → session user
    - Data user disimpan ke tabel users + user_tiers
    
    Google OAuth (fallback):
    - Tetap mendukung code exchange seperti sebelumnya
    """
    ...
    # ========== STEP 2: Handle Magic Link Callback (token_hash) ==========
    ...
    if "token_hash" in query_params:
        ...
        try:
            supabase = get_supabase_client()
            # Verifikasi OTP token dari magic link
            ...
            if auth_response and auth_response.user:
                ...
                send_log(f"MAGIC LINK: {st.session_state.user_name} ({st.session_state.user_email})")
                
                st.query_params.clear()
                st.rerun()
        except Exception as e:
            print(f"DEBUG: Magic Link failed - {str(e)[:100]}")
    
    # ========== STEP 3: Handle Google OAuth Callback (code) - FALLBACK ==========
    if "code" in query_params:
        ...
        try:
            ...
            if auth_response and auth_response.user:
                ...
                sync_user_to_supabase(user.id, user.email or "", st.session_state.user_name, st.session_state.user_avatar)
                send_log(f"AUTH (Google): {st.session_state.user_name} ({st.session_state.user_email})")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if user_response.status_code == 200:
                    ...
                    send_log(f"AUTH (REST): {user_name} ({user_email})")
...
# Handle Auth Callback (Magic Link + Google Fallback)
...
translations = {
    "ID": {
        ...
        "login_magic": "MAGIC LINK LOGIN",
        "login_email_placeholder": "ENTER EMAIL ADDRESS",
        "login_send_btn": "SEND MAGIC LINK",
        "login_check_email": "Check your email for the magic link!",
        ...
        "sign_in_desc": "Enter email to receive a secure magic link",
...
<style>
    ...
    * {
    ...
    .magic-link-form-container::before {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
    ...
    .magic-link-input input::placeholder {
    ...
    .magic-link-btn button:hover {
        ...
        box-shadow: 0 0 20px rgba(0, 255, 136, 0.2), 0 0 40px rgba(0, 255, 136, 0.05) !important;
        color: #ffffff !important;
        transform: translateY(-1px);
    }
...
with st.sidebar:
    ...
    if st.session_state.auth_session and st.session_state.user_name:
        ...
        # User not logged in - MAGIC LINK LOGIN (CYBER FORM)
        st.markdown(f"""
        ...
        st.markdown('<div class="magic-link-form-container">', unsafe_allow_html=True)

        with st.form("magic_link_login_form"):
            ...
            st.markdown('<div class="magic-link-input">', unsafe_allow_html=True)
            # Inject CSS untuk input
            st.markdown("""<style>
            ...
            if submitted and email_input:
                try:
                    ...
                    redirect_to = st.secrets.get("REDIRECT_URL", "https://aerovulpis.streamlit.app")
                    resp = supabase_auth.auth.sign_in_with_otp({
                    ...
                    st.markdown(f"""
                    <div class="magic-link-status magic-link-success">
                        [LINK SENT] Magic link telah dikirim ke<br>
                        <span style="color:#00d4ff;">{email_input.strip()}</span><br>
                        <span style="font-size:8px;color:#556680;">Cek inbox atau spam folder</span>
                    </div>
                    """, unsafe_allow_html=True)
                except Exception as e:
                    st.markdown(f"""
                    <div class="magic-link-status magic-link-error">
                        [ERROR] Gagal mengirim magic link<br>
                        <span style="font-size:8px;color:#ff2a6d;">{str(e)[:100]}</span>
                    </div>
                    """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        <div class="cyber-divider">
            <span>{t.get('or_continue_with', 'OR CONTINUE WITH')}</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        try:
            ...
            <a href="{google_auth_link}" target="_self" style="text-decoration:none;">
                <div class="google-login-btn" style="margin-top:4px;">
...
# ==============================================================================
# 11. HELP & SUPPORT (DIPERBAIKI - CENTERED CARDS + TELEGRAM)
# ==============================================================================
    ...
    with st.expander("AKTIVASI LISENSI & LEVEL"):
        ...
        **Cara Masuk:** Masukkan email di sidebar → klik "SEND MAGIC LINK" → cek inbox.
