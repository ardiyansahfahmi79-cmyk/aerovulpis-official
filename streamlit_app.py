...
def sync_user_to_supabase(user_id, email, name, avatar=""):
    """
    Menyinkronkan user dari auth.users ke tabel public.users dan public.user_tiers.
    Dipanggil setiap kali login/register berhasil.
    """
    try:
        supabase = get_supabase_client()
...
def check_email_exists_admin(email):
    """
    Mengecek apakah email sudah terdaftar di tabel public.users.
    Tabel ini selalu disinkronkan saat registrasi/login via sync_user_to_supabase().
    Return (bool, user_id_or_None).
    """
    try:
        supabase = get_supabase_client()
        res = supabase.table("users").select("id").eq("email", email.strip().lower()).execute()
...
def delete_user_with_verification(email, password):
    """
    Verifikasi kredensial user (email + password) sebelum menghapus akun.
    Jika verifikasi berhasil, hapus menggunakan service_role.
    Ini memastikan hanya pemilik akun yang bisa menghapus.
    """
    try:
        supabase = get_supabase_client()
        ...
        # Hapus dari auth.users menggunakan admin client
        supabase_admin = get_supabase_admin()
        ...
        return True, "[SYSTEM] Akun berhasil dihapus permanen setelah verifikasi."
    except Exception as e:
...
# CSS (TIDAK BERUBAH - persis seperti file asli)
st.markdown("""
<style>
...
# ##############################################################################
# SIDEBAR CONTROL CENTER (FINAL - AUTH FIX)
# ##############################################################################
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        with st.form("email_password_login_form"):
            ...
            if login_submitted and email_input and password_input:
                try:
                    ...
                    if resp and resp.user:
                        ...
                        st.error("[AUTH] INVALID EMAIL OR PASSWORD")
                except Exception as e:
                    error_msg = str(e).lower()
                    if "email not confirmed" in error_msg:
                        st.error("[AUTH] Email belum dikonfirmasi. Silakan cek inbox/spam Anda dan klik link konfirmasi.")
                    elif "invalid login credentials" in error_msg:
                        st.error("[AUTH] Email atau password salah. Gunakan LUPA PASSWORD jika Anda yakin email sudah benar.")
                    elif "user not found" in error_msg:
                        st.error("[AUTH] User tidak ditemukan. Silakan REGISTER terlebih dahulu.")
                    else:
            ...
            if signup_submitted and email_input and password_input:
                ...
                if len(password_input) < 6:
                    ...
                    # Cek apakah email sudah terdaftar di tabel users (sinkronisasi dengan auth)
                    already_exists, _ = check_email_exists_admin(email_clean)
                    if already_exists:
                        st.warning("[SYSTEM] Email sudah terdaftar. Silakan LOGIN atau gunakan LUPA PASSWORD / DELETE ACCOUNT.")
                    else:
                        try:
                            supabase_auth = get_supabase_client()
                            ...
                            # Cek resp.user (bukan resp.session) untuk menentukan keberhasilan
                            if resp and resp.user:
                                st.session_state.auth_session = resp.session.access_token if resp.session else "active"
                                ...
                                st.error("[SYSTEM] Registrasi gagal. Silakan coba lagi.")
                        except Exception as e:
                            err = str(e).lower()
                    ...
                    if st.button("Kirim Link Reset", key="send_reset_btn", width='stretch'):
                        if reset_email:
                            try:
                                ...
                                st.success(f"Link reset password telah dikirim ke {reset_email}. Silakan cek inbox/spam Anda.")
        ...
        # Form Delete Account (VERIFIKASI PASSWORD)
                ...
                with col_d1:
                    if st.button("HAPUS PERMANEN", key="confirm_delete_btn", width='stretch', type="primary"):
                        if del_email and del_pass:
                            ...
                            st.warning("Masukkan email dan password untuk verifikasi.")
                with col_d2:
...
# ==============================================================================
# 10. TINGKATKAN LEVEL (UPGRADE PAGE) - QR DARI SECRETS & MODAL 300px
# ==============================================================================
    ...
    # Build HTML cards - gabungkan dalam satu string
    ...
    # MODAL PEMBAYARAN (MENGGUNAKAN QR DARI SECRETS, MAX-WIDTH 300px)
    if st.session_state.show_payment_modal:
        ...
        # Ambil URL gambar QR dari secrets
        qr_image_url = st.secrets.get("QRIS_IMAGE_URL", "")
        if qr_image_url:
            qr_img_html = f'<img src="{qr_image_url}" alt="QRIS Pembayaran" style="width:150px;height:150px;border-radius:4px;">'
        else:
            qr_img_html = '<div class="qr-container"><p style="color:black;padding:20px;">QR Code belum dikonfigurasi</p></div>'
...
Mohon diproses. Berikut bukti pembayaran (lampirkan gambar)."""
        ...
        <div class="payment-modal-overlay" id="payment-modal">
            <div class="payment-modal">
                <span class="close-modal-btn" onclick="document.getElementById('payment-modal').style.display='none';">&times;</span>
                ...
                <div class="payment-steps">
                    <span>STEP 01</span> Klik tombol BAYAR VIA TELEGRAM di bawah<br>
                    <span>STEP 02</span> Sampaikan pesan dengan format yang tersedia<br>
                    <span>STEP 03</span> Lampirkan bukti pembayaran
                </div>
                ...
                <div style="text-align:left;background:rgba(0,20,40,0.5);padding:10px;border-radius:4px;margin:12px 0;font-family:Share Tech Mono;font-size:8px;color:#8899bb;white-space:pre-wrap;">{telegram_text}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        col1, col2, col3 = st.columns([1, 1, 1])
        with col1:
            telegram_url = "https://t.me/WartaPrime"
            st.markdown(f'<a href="{telegram_url}" target="_blank"><button style="width:100%;background:#0088cc;color:white;border:none;padding:8px;font-family:Orbitron;font-size:9px;letter-spacing:1px;">BAYAR VIA TELEGRAM</button></a>', unsafe_allow_html=True)
        with col2:
            st.markdown(f"""
            <button onclick="navigator.clipboard.writeText(`{telegram_text}`);" style="width:100%;background:rgba(0,212,255,0.2);border:1px solid #00d4ff;color:#00d4ff;padding:8px;font-family:Orbitron;font-size:9px;letter-spacing:1px;">COPY TEXT</button>
            """, unsafe_allow_html=True)
        with col3:
            if st.button("TUTUP", key="close_payment_modal", width='stretch'):
