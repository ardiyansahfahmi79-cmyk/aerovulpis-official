...
def get_supabase_client():
    ...
    Digunakan untuk operasi yang memerlukan RLS (Row Level Security)
    """
...
def get_supabase_admin():
    ...
    Seperti update tier user dan aktivasi kunci
    """
...
def send_log(pesan):
    """
    Mencatat log aktivitas sistem ke tabel logs_aktivitas di Supabase
...
def cleanup_logs():
    ...
    Untuk menghemat storage database
    """
    try:
        supabase = get_supabase_client()
...
def cache_market_price(symbol, price, change_pct=0.0):
    """
    Menyimpan harga pasar terbaru ke tabel market_prices di Supabase
    Menggunakan upsert agar data selalu update (insert if new, update if exists)
    """
    try:
        supabase = get_supabase_client()
        data = {
...
def get_cached_market_price(symbol):
    """
    Mengambil harga terbaru dari cache Supabase
...
def get_cached_market_price_full(symbol):
    """
    Mengambil data harga lengkap dari cache untuk keperluan Smart Alert System
...
def cleanup_old_data():
    ...
    Untuk menjaga ukuran database tetap optimal
    """
    try:
        supabase = get_supabase_client()
...
def get_user_tier(user_id):
    ...
    Return: (tier, expired_at) atau ('free', None) jika tidak ditemukan
    """
    if not user_id:
        return "free", None
    try:
            ...
            if expired_at:
                try:
                    ...
                    if datetime.now(pytz.UTC) > expired_date:
                        # Tier sudah expired, update ke free menggunakan service_role
...
def activate_key(user_id, key_code):
    ...
    2. Jika valid, ambil data tier dan durasi
    3. Update tabel user_tiers dengan tier baru dan expired date
    ...
    try:
        ...
        # Hitung expired date dalam UTC
        expired_at = (datetime.now(pytz.UTC) + timedelta(days=duration_days)).isoformat()
        
        # Gunakan service_role untuk bypass RLS saat update tier
...
def sync_user_to_supabase(user_id, email, name, avatar=""):
    ...
    Jika user baru: insert user + beri tier free
    """
    try:
        supabase = get_supabase_client()
...
def get_cached_ai_analysis(asset_name, timeframe):
    ...
    Cache berlaku selama 5 menit untuk menghemat API calls
    Return: string analysis atau None jika cache tidak tersedia/expired
    """
    try:
        supabase = get_supabase_client()
...
def cache_ai_analysis(asset_name, timeframe, analysis):
    """
    Menyimpan hasil analisis AI ke cache Supabase
    
    Digunakan untuk berbagi hasil analisis antar user
    sehingga menghemat pemakaian API key
    """
    try:
        supabase = get_supabase_client()
...
def handle_google_oauth():
    ...
    Alur:
    1. Cek query params untuk 'code' (callback dari Google)
    2. Exchange authorization code untuk session
    3. Sinkronisasi user ke database
    4. Set session state dengan data user
    
    GOOGLE 403 FIX - Pastikan konfigurasi berikut di Google Cloud Console:
    - Authorized JavaScript origins:
      https://bqdugkmmnbxlftuxqtdg.supabase.co
    - Authorized redirect URIs:
      https://bqdugkmmnbxlftuxqtdg.supabase.co/auth/v1/callback
    
    Dan di Supabase Dashboard → Authentication → Providers → Google:
    - Enable = ON
    - Client ID & Client Secret dari Google Console
    - Skip nonce checks = OFF
    """
    # Handle OAuth callback (user kembali dari halaman Google)
    query_params = st.query_params
    if "code" in query_params:
        code = query_params["code"]
        try:
            supabase = get_supabase_client()
            # Exchange authorization code untuk mendapatkan session
            ...
            if auth_response and auth_response.user:
                ...
                # Sinkronisasi data user ke database
                ...
                # Ambil tier user dari database
                st.session_state.user_tier, _ = get_user_tier(user_id)
                
                # Bersihkan query params agar URL tidak menyimpan code
...
# =====================================================================
# ICMARKET PRICE FETCH (AKURAT DARI BROKER ASLI)
# =====================================================================
def get_icmarket_price(symbol):
    ...
    - XAUUSD, XAGUSD (Gold & Silver) → 2 desimal
    - EURUSD, GBPUSD, USDJPY, AUDUSD, USDCHF (Forex) → 4 desimal
    - BTCUSD, ETHUSD (Crypto) → 2 desimal
    - SOLUSD, XRPUSD, BNBUSD (Alt Crypto) → 4 desimal
    
    Return: dict dengan keys price, bid, ask, spread, source
    """
    # Mapping instrumen ke ID cTrader
    ctrader_map = {
        "XAUUSD": "1",
        "XAGUSD": "2",
        "EURUSD": "3",
    ...
    # Ambil credentials dari secrets atau environment variables
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Format harga berdasarkan jenis instrumen (broker asli)
            if symbol in ["XAUUSD"]:
                formatted_price = round(mid_price, 2)      # Contoh: 4756.00
            elif symbol in ["XAGUSD"]:
                formatted_price = round(mid_price, 2)      # Contoh: 34.50
            elif symbol in ["BTCUSD", "ETHUSD"]:
                formatted_price = round(mid_price, 2)      # Contoh: 67250.00
            elif symbol in ["SOLUSD", "XRPUSD", "BNBUSD"]:
                formatted_price = round(mid_price, 4)      # Contoh: 145.8000
            else:
                formatted_price = round(mid_price, 4)      # Forex: 1.0850
            
            # Hitung spread dalam pip
...
# Inisialisasi sistem pada saat aplikasi dimulai
cleanup_logs()
cleanup_old_data()
...
def get_market_data(ticker_symbol):
    ...
    1. IC Market cTrader API (XAUUSD, XAGUSD, Forex, Crypto)
    2. Supabase Cache (3 detik freshness)
    ...
    try:
        ...
        if inst_name in ctrader_instruments:
            ...
            if ctrader_data:
                # Update cache dengan harga dari broker
...
def get_historical_data(ticker_symbol, period="1mo", interval="1h"):
    ...
    Parameters:
    - ticker_symbol: kode ticker (contoh: GC=F, EURUSD=X)
    - period: rentang waktu (1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y)
    - interval: interval candle (1m, 5m, 15m, 30m, 1h, 1d, 1wk)
...
def add_technical_indicators(df):
    ...
    - Momentum: RSI(14), MACD, Stochastic K/D, CCI(20), Williams %R(14), MFI(14),
                 TRIX(15), ROC(12), Awesome Oscillator(5/34)
    - Volatility: Bollinger Bands(20,2), ATR(14)
    - Trend Strength: ADX(14) dengan +DI dan -DI
    ...
    # --- Additional Indicators (menggunakan library ta) ---
...
def get_sentinel_analysis(asset_name, market_data, df, signal, reasons):
    ...
    Primary Model: Hermes 3 405B (via OpenRouter)
    Companion Model: Qwen3 Next 80B Instruct (gratis, untuk detail tambahan)
    Backup Models (jika primary limit):
    1. Ling-2.6-Flash (DeepSeek Chat)
    2. LFM2.5-1.2B-Thinking (Liquid)
    3. Minimax M2.5
    
    Hasil analisis di-cache 5 menit untuk menghemat API calls
    """
    ...
    # Get news for fundamental context
...
"""
    ...
    # 1. Try Primary Model (Hermes 405B)
    ...
    # 2. Add Companion Insights (Qwen3 Next 80B)
    if analysis:
...
def get_deep_analysis(asset_name, market_data, df, signal, reasons):
    ...
    Menggunakan Llama 3.3 70B via Groq untuk analisis teknikal mendalam
    Dilengkapi dengan level entry, stop loss, dan take profit spesifik
    """
    if not client:
        return "ERROR: SYSTEM CONFIGURATION REQUIRED"
...
Use markdown formatting. Maximum 2000 characters. Focus on actionable insights."""
...
def get_news_data(category="General", max_articles=10):
    """
    Mengambil berita dari multiple sources dengan intelligent caching
    
    Sumber berita:
    1. Marketaux API (primary)
    2. Tiingo API (secondary)
    3. NewsAPI (fallback)
    
    Cache berlaku 5 menit dengan force refresh mechanism
    Berita difilter 6 jam terakhir untuk kesegaran
    """
    from news_cache_manager import initialize_news_cache, should_update_news, get_cached_news, update_news_cache
    ...
    # Return cache if not refreshing and cache is valid
    ...
    # Convert timestamps to WIB (Jakarta time)
...
def check_smart_alerts():
    ...
    Flow:
    1. Ambil semua alert yang belum triggered dari session_state
    2. Cek harga terkini dari Supabase cache
    3. Bandingkan dengan target harga
    ...
    # Collect unique instruments that need price checking
    ...
    # Map instrument names to ticker symbols
...
with st.sidebar:
    ...
    if st.session_state.auth_session and st.session_state.user_name:
        # User sudah login - tampilkan info user
        ...
        # User belum login - tampilkan Google Sign-In
        st.markdown(f"""
...
# Jalankan smart alert checker setiap rerun
check_smart_alerts()
    ...
    with col_chart:
        ...
        if st.button(t['sentinel_btn'], key="sentinel_pro_btn", use_container_width=True):
            ...
            if market and not df.empty:
                ...
                # Tampilkan 3D loading animation
...
# =====================================================================
# 7. SMART ALERT CENTER (NO DUPLICATE - HANYA WIDGET)
# =====================================================================
    ...
    # HANYA PANGGIL WIDGET - Tidak ada current price duplikat
    smart_alert_widget()
...
# =====================================================================
    ...
    with st.expander("SENTINEL PRO INTELLIGENCE", expanded=True):
        ...
        **Data Sources:** IC Market cTrader API (primary for XAUUSD, XAGUSD, Forex, Crypto) with accurate broker pricing, with automatic fallback to system cache and global market data.

        **Usage:** Select instrument and timeframe from the sidebar, navigate to the Sentinel page, and click "INITIATE DEEP ANALYSIS PRO" to generate a full intelligence report.
        """)
    ...
    with st.expander("LIVE DASHBOARD"):
        ...
        **Price Feed Priority:**
        1. IC Market cTrader API (XAUUSD, XAGUSD, Forex pairs, Major Crypto)
    ...
    with st.expander("SIGNAL ANALYSIS"):
        ...
        **Signal Color Coding:** Green = Bullish | Red = Bearish | Yellow = Neutral
    ...
    with st.expander("CHATBOT AI"):
        st.markdown("""
        **Neural Assistant** powered by the AeroVulpis Engine, providing real-time analysis and insights.
        ...
        **Daily Limits by Tier:**
        ...
        - MONTHLY: 200 messages/day
        - 6 MONTHS PRO: 500 messages/day
        - ULTIMATE: Unlimited
        """)
    ...
    with st.expander("RISK MANAGEMENT"):
        ...
        **Pillar 1 - Trading Rules:** Define clear stop loss levels and trading parameters before entering any position.
        **Pillar 2 - Position Sizing:** Calculate optimal position size based on account balance and risk tolerance.
        **Pillar 3 - Confidence Scores:** Utilize real-time technical strength assessment to filter high-probability setups.
        ...
        **Return Simulator Features:**
        ...
        - Daily maximum loss and profit configuration
        - Projected performance: Weekly, Monthly, and Yearly Net P/L, Return %, and Final Balance
    ...
    with st.expander("LICENSE & TIER SYSTEM"):
        ...
        **Google Authentication Setup:**
        ...
        2. Supabase Dashboard → Authentication → Providers → Google → Enable + Fill Client ID & Secret
        """)
