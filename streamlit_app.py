...
# ##############################################################################
# DELETE USER BY EMAIL (MENGGUNAKAN SERVICE_ROLE, DENGAN METHOD CLIENT)
# ##############################################################################
...
def delete_user_by_email(email):
    ...
    try:
        supabase_admin = get_supabase_admin()
        # List users dengan filter email
        list_response = supabase_admin.auth.admin.list_users(page=1, per_page=1, email=email)
        if list_response and hasattr(list_response, 'users') and list_response.users:
            user_id = list_response.users[0].id
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        if st.session_state.get("show_delete_account", False):
            with st.container():
                st.warning("PERHATIAN: Ini akan menghapus akun Anda secara permanen. Semua data (tier, aktivitas, cache) akan hilang.")
