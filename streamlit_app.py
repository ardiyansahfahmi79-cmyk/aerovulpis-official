...
# ##############################################################################
# DELETE USER FROM SUPABASE AUTH (MENGGUNAKAN service_role)
# ##############################################################################

def delete_user_completely(email, password):
    """
    Menghapus user sepenuhnya dari Supabase Auth.
    Langkah:
    1. Login dengan kredensial user untuk mendapatkan user_id.
    2. Gunakan service_role key untuk menghapus user dari auth.users.
    """
    try:
        supabase = get_supabase_client()
        # Login dulu untuk dapatkan user_id
        resp = supabase.auth.sign_in_with_password({"email": email.strip(), "password": password})
        if resp and resp.user:
            user_id = resp.user.id
            # Gunakan admin client untuk hapus dari auth.users
            supabase_admin = get_supabase_admin()
            supabase_admin.auth.admin.delete_user(user_id)
            # Hapus juga dari tabel kustom
            supabase.table("users").delete().eq("id", user_id).execute()
            supabase.table("user_tiers").delete().eq("user_id", user_id).execute()
            return True, "User berhasil dihapus dari sistem."
        else:
            return False, "Tidak dapat menemukan user dengan kredensial tersebut."
    except Exception as e:
...
# Market data functions (Finnhub, yfinance, etc.) - tidak berubah
...
# ##############################################################################
# SIDEBAR CONTROL CENTER (FINAL - DENGAN DELETE ACCOUNT & LUPA PASSWORD)
# ##############################################################################
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        # Form Delete Account (HAPUS PERMANEN DARI AUTH.USERS)
        if st.session_state.get("show_delete_account", False):
            with st.container():
                ...
                del_pass = st.text_input("Password akun", key="del_pass", type="password", placeholder="Masukkan password")
                col_d1, col_d2 = st.columns(2)
                with col_d1:
                    if st.button("HAPUS AKUN", key="confirm_delete_btn", width='stretch', type="primary"):
                        if del_email and del_pass:
                            success, msg = delete_user_completely(del_email.strip(), del_pass)
                            if success:
                                st.success(msg)
