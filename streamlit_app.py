...
def get_supabase_client(): return create_client(url, key)
def get_supabase_admin(): return create_client(url, service_role_key)
...
def cache_market_price(symbol, price, change_pct=0.0):
    try:
        supabase = get_supabase_client()
        data = {"instrument": symbol, "price": price, "change_pct": change_pct, "updated_at": datetime.now(pytz.timezone('Asia/Jakarta')).isoformat()}
...
def get_cached_market_price(symbol):
    try:
        ...
        if res.data: return res.data[0]["price"]
    except Exception: pass
    return None
...
def get_cached_market_price_full(symbol):
    try:
        ...
        if res.data:
            return {"price": res.data[0].get("price", 0), "change_pct": res.data[0].get("change_pct", 0), "updated_at": res.data[0].get("updated_at", "")}
    except Exception: pass
    return None
...
def get_user_tier(user_id):
    if not user_id: return "free", None
    try:
        supabase = get_supabase_client()
...
def activate_key(user_id, key_code):
    if not user_id or not key_code: return False, "IDENTITY VERIFICATION REQUIRED"
    try:
        supabase = get_supabase_client()
        res = supabase.table("activation_keys").select("*").eq("key_code", key_code.upper().strip()).eq("is_used", False).execute()
        if not res.data: return False, "INVALID OR EXPIRED LICENSE KEY"
        key_data = res.data[0]
        ...
        supabase_admin.table("user_tiers").upsert({"user_id": user_id, "tier": tier, "expired_at": expired_at, "activated_at": datetime.now(pytz.UTC).isoformat()}).execute()
        supabase.table("activation_keys").update({"is_used": True, "used_by": user_id, "used_at": datetime.now(pytz.UTC).isoformat()}).eq("key_code", key_code.upper().strip()).execute()
...
def sync_user_to_supabase(user_id, email, name, avatar=""):
    try:
        ...
        if existing.data:
            supabase.table("users").update({"email": email, "name": name, "avatar": avatar, "last_login": datetime.now(pytz.UTC).isoformat()}).eq("id", user_id).execute()
        else:
            supabase.table("users").insert({"id": user_id, "email": email, "name": name, "avatar": avatar, "created_at": datetime.now(pytz.UTC).isoformat(), "last_login": datetime.now(pytz.UTC).isoformat()}).execute()
            try:
                supabase_admin = get_supabase_admin()
                supabase_admin.table("user_tiers").insert({"user_id": user_id, "tier": "free", "activated_at": datetime.now(pytz.UTC).isoformat()}).execute()
            except Exception:
                try: supabase.table("user_tiers").insert({"user_id": user_id, "tier": "free", "activated_at": datetime.now(pytz.UTC).isoformat()}).execute()
                except Exception: pass
    except Exception: pass
...
def get_cached_ai_analysis(asset_name, cache_type="sentinel"):
    ...
    try:
        ...
        if res.data: return res.data[0]["analysis"]
    except Exception: pass
    return None
...
def cache_ai_analysis(asset_name, analysis, cache_type="sentinel"):
    ...
    try:
        supabase = get_supabase_client()
        supabase.table(table_name).insert({"asset_name": asset_name, "analysis": analysis, "created_at": datetime.now(pytz.UTC).isoformat()}).execute()
    except Exception: pass
    ...
    if not st.session_state.get("auth_session"):
        try:
            ...
            if session and session.user:
                ...
                return True
        except Exception as e: print(f"DEBUG: No existing session - {str(e)[:100]}")
    return False
...
def fetch_finnhub_price(symbol):
    ...
    finnhub_map = {"XAUUSD":"XAUUSD","XAGUSD":"XAGUSD","EURUSD":"EURUSD","GBPUSD":"GBPUSD","USDJPY":"USDJPY","AUDUSD":"AUDUSD","USDCHF":"USDCHF","USDCAD":"USDCAD","NZDUSD":"NZDUSD","BTCUSD":"BTCUSD","ETHUSD":"ETHUSD","SOLUSD":"SOLUSD","XRPUSD":"XRPUSD","BNBUSD":"BNBUSD"}
    finn_symbol = finnhub_map.get(symbol)
    if not finn_symbol: return None
    try:
        ...
        if finn_symbol in ["XAUUSD","XAGUSD","BTCUSD","ETHUSD","SOLUSD","XRPUSD","BNBUSD"]:
            ...
            if res and res.get('c') and len(res['c'])>0: return {"price": float(res['c'][-1]), "source": "FINNHUB"}
        else:
            ...
            if res and res.get('quote'):
                ...
                if price: return {"price": float(price), "source": "FINNHUB"}
    except Exception: pass
    return None
...
def format_price_display(price, instrument_name):
    ...
    elif any(fx in name_upper for fx in ["EUR","GBP","CHF","JPY","AUD","NZD","CAD"]): return f"{price:,.4f}"
    elif any(idx in name_upper for idx in ["NASDAQ","S&P","DOW","DAX","IHSG","SP500"]): return f"{price:,.2f}"
...
st.set_page_config(layout="wide", page_title="AEROVULPIS V3.5", page_icon="https://files.manuscdn.com/user_upload_by_module/session_file/310519663520709901/oOIKIIkSvIdagiSw.png", initial_sidebar_state="expanded")
cleanup_logs()
cleanup_old_data()
cleanup_ai_cache()
...
translations = {
    "ID": {
        ...
        "sentinel_intel": "INTELLIGENCE REPORT", "sentinel_placeholder": "Initialize Deep Analysis Pro to generate intelligence report",
        "news_filter": "CATEGORY FILTER", "news_updated": "Live feed from global financial networks | Updated hourly",
        "economic_title": "GLOBAL ECONOMIC SCANNER", "economic_subtitle": "Real-Time High Impact Event Detection Active",
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    .payment-modal { background: linear-gradient(160deg, rgba(0,15,38,0.98), rgba(0,5,18,0.99)); border: 2px solid rgba(0,212,255,0.3); border-radius: 10px; padding: 35px 30px; max-width: 520px; width: 90%; text-align: center; position: relative; box-shadow: 0 0 100px rgba(0,212,255,0.15), 0 0 200px rgba(0,212,255,0.05); }
    ...
    .payment-modal-title { font-family: 'Orbitron', sans-serif; font-size: 18px; font-weight: 800; color: #00d4ff; letter-spacing: 4px; margin-bottom: 5px; }
    .payment-modal-subtitle { font-family: 'Share Tech Mono', monospace; font-size: 8px; color: #557799; letter-spacing: 3px; margin-bottom: 20px; }
    .qr-container { background: #ffffff; padding: 15px; border-radius: 6px; display: inline-block; margin-bottom: 15px; border: 2px solid rgba(0,212,255,0.3); box-shadow: 0 0 30px rgba(0,212,255,0.2); }
    .payment-info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin: 12px 0; text-align: left; }
    .payment-info-item { background: rgba(0,20,50,0.5); border: 1px solid rgba(0,212,255,0.1); border-radius: 3px; padding: 8px 10px; }
    .payment-info-label { font-family: 'Orbitron', sans-serif; font-size: 6px; color: #557799; letter-spacing: 1.5px; text-transform: uppercase; }
    .payment-info-value { font-family: 'Share Tech Mono', monospace; font-size: 10px; color: #00ff88; }
    .trust-badge-container { display: flex; justify-content: center; gap: 15px; margin: 12px 0; flex-wrap: wrap; }
    .trust-badge { background: rgba(0,212,255,0.04); border: 1px solid rgba(0,212,255,0.12); border-radius: 3px; padding: 6px 14px; font-family: 'Share Tech Mono', monospace; font-size: 7px; color: #6688aa; letter-spacing: 1px; }
    .trust-badge .highlight { color: #00d4ff; font-weight: 700; }
    .payment-steps { text-align: left; font-family: 'Rajdhani', sans-serif; font-size: 10px; color: #8899bb; margin: 10px 0; line-height: 1.7; }
    .payment-steps span { color: #00d4ff; font-family: 'Orbitron', sans-serif; font-size: 8px; margin-right: 5px; }
    .dynamihatch-watermark { font-family: 'Orbitron', sans-serif; font-size: 8px; color: rgba(255,255,255,0.03); position: absolute; bottom: 10px; right: 20px; letter-spacing: 3px; }
</style>
...
# ##############################################################################
# MARKET DATA FUNCTIONS (Finnhub -> Cache -> yfinance)
# ##############################################################################
...
def get_market_data(ticker_symbol):
    try:
        ...
        if finnhub_result and finnhub_result.get("price", 0) > 0:
            ...
            return {"price": finnhub_result["price"], "change": 0, "change_pct": 0, "source": "FINNHUB", "spread": 0}
...
def get_weighted_signal(df):
    ...
    reasons = []
    rsi_val = latest["RSI"]
    if rsi_val < 30: bullish_count += 1; reasons.append(f"RSI OVERSOLD [{rsi_val:.1f}]")
    elif rsi_val > 70: bearish_count += 1; reasons.append(f"RSI OVERBOUGHT [{rsi_val:.1f}]")
    else: neutral_count += 1; reasons.append(f"RSI NEUTRAL [{rsi_val:.1f}]")
    ...
    if latest["Close"] > latest["SMA50"]: bullish_count += 1; reasons.append("PRICE ABOVE SMA50")
    else: bearish_count += 1; reasons.append("PRICE BELOW SMA50")
    if latest["Close"] > latest["SMA200"]: bullish_count += 1; reasons.append("PRICE ABOVE SMA200")
    else: bearish_count += 1; reasons.append("PRICE BELOW SMA200")
...
TECHNICAL DATA: RSI (14): {latest.get('RSI',0):.2f} | MACD: {latest.get('MACD',0):.4f} | Signal Line: {latest.get('Signal_Line',0):.4f}
ATR (14): {latest.get('ATR',0):.4f} | ADX (14): {latest.get('ADX',0):.2f} | Stochastic K: {latest.get('Stoch_K',0):.2f}
Bollinger Upper: {latest.get('BB_Upper',0):.4f} | Bollinger Lower: {latest.get('BB_Lower',0):.4f}
TECHNICAL REASONS: {', '.join(reasons)}
MARKET NEWS: {news_context}
REQUIRED OUTPUT: [KEY LEVELS] Support/Resistance | [FUNDAMENTAL INSIGHT] | [BULLISH SCENARIO] Entry/Target/SL/RR | [BEARISH SCENARIO] | [FINAL VERDICT]
RULES: Indonesian, max 320 words, balanced, April 2026 market."""
    ...
    if not analysis:
        ...
        for i, model in enumerate(BACKUP_MODELS):
            ...
            if analysis: analysis = f"[BACKUP SYSTEM ACTIVE: {backup_names[i]}]\n\n" + analysis; break
    if not analysis: return "ALL NEURAL SYSTEMS AT CAPACITY | PLEASE RETRY IN A FEW MINUTES"
...
def market_session_status():
    ...
    st.markdown('<div class="session-container"><h2 class="cyber-glow-text" style="text-align:center;font-size:22px;margin-bottom:25px;letter-spacing:5px;">GLOBAL MARKET SESSIONS</h2>', unsafe_allow_html=True)
    active_sessions = []
    ...
    if "ASIAN SESSION" in active_sessions and len(active_sessions) == 1: strategy_text = "RANGE TRADING PROTOCOL"; strategy_detail = "Focus on liquidity sweeps and Asian range breakout patterns"
    elif is_golden: strategy_text = "HIGH VOLATILITY PROTOCOL"; strategy_detail = "Order block mitigations and FVG entries. Tight spreads, maximum momentum"
    elif "EUROPEAN SESSION" in active_sessions: strategy_text = "TREND FOLLOWING PROTOCOL"; strategy_detail = "London breakout patterns. Monitor displacement moves for entry confirmation"
    elif "AMERICAN SESSION" in active_sessions: strategy_text = "REVERSAL PROTOCOL"; strategy_detail = "NY open manipulation watch. Late session reversals probability elevated"
...
def generate_qr_base64(data_string, size=180):
    try:
...
def get_news_data(category="General", max_articles=10):
    ...
    if not force_refresh and not should_update_news(category):
        ...
        if cached_news: return cached_news, None
                ...
                for item in res_c.get('news', []):
                    if item.get('url') and item['url'] not in urls_terpakai:
                        if category == "Forex":
                            ...
                            if not any(kw.lower() in title_lower for kw in forex_keywords): continue
        ...
        try:
            ...
            if res_n.get('articles'):
                for item in res_n.get('articles', []):
                    if item.get('url') and item['url'] not in urls_terpakai:
                        berita_final.append({'publishedAt': item.get('publishedAt', datetime.now().isoformat()), 'title': item.get('title', 'NO TITLE'), 'description': item.get('description', ''), 'source': item.get('source', {}).get('name', 'NEWS NETWORK'), 'url': item['url']})
                        urls_terpakai.add(item['url'])
        except Exception: pass

    if not berita_final:
        cached_news = get_cached_news(category)
        if cached_news: return cached_news, "DISPLAYING CACHED DATA | LIVE FEED UNAVAILABLE"
...
# ##############################################################################
# SIDEBAR CONTROL CENTER (DENGAN PROTEKSI TIER & LOGIN)
# ##############################################################################
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        st.stop()  # Hentikan rendering sidebar & main content

    # --- Jika SUDAH LOGIN: tampilkan user info & menu sesuai tier ---
    ...
    # --- Cegah akses ke menu premium untuk user free ---
    if not user_is_premium and "(Premium)" in menu_selection:
...
# ==============================================================================
# 1. AEROVULPIS SENTINEL PRO (PREMIUM)
# ==============================================================================
if menu_selection == "AeroVulpis Sentinel" or menu_selection == "AeroVulpis Sentinel (Premium)":
...
# ==============================================================================
# 2. LIVE DASHBOARD (TERBUKA UNTUK SEMUA TIER)
# ==============================================================================
...
# ==============================================================================
# 4. MARKET SESSIONS (PREMIUM)
# ==============================================================================
elif menu_selection == "Market Sessions" or menu_selection == "Market Sessions (Premium)":
    if not user_is_premium:
        st.error(f"**{t['premium_lock']}**")
        st.info(t['premium_msg'])
        st.stop()
...
# ==============================================================================
# 5. MARKET NEWS (PREMIUM)
# ==============================================================================
elif menu_selection == "Market News" or menu_selection == "Market News (Premium)":
    if not user_is_premium:
        st.error(f"**{t['premium_lock']}**")
        st.info(t['premium_msg'])
        st.stop()
    ...
    with col_refresh:
        if st.button(t['force_refresh'], width='stretch', key="force_news_refresh"):
            if "last_news_fetch" in st.session_state: st.session_state.last_news_fetch = {}
            st.cache_data.clear()
            st.rerun()
    
    articles, error = get_news_data(selected_news_cat, 10)
    if error and not articles: st.error(error)
    elif articles:
        for a in articles:
            st.markdown(f"""
            <div class="news-card">
            ...
            """, unsafe_allow_html=True)
    else: st.info(t['no_news'])

# ==============================================================================
# 7. SMART ALERT CENTER (PREMIUM)
# ==============================================================================
elif menu_selection == "Smart Alert Center" or menu_selection == "Smart Alert Center (Premium)":
    if not user_is_premium:
        st.error(f"**{t['premium_lock']}**")
        st.info(t['premium_msg'])
        st.stop()
...
# ==============================================================================
# 9. RISK MANAGEMENT (PREMIUM)
# ==============================================================================
elif menu_selection == "Risk Management" or menu_selection == "Risk Management (Premium)":
    if not user_is_premium:
        st.error(f"**{t['premium_lock']}**")
        st.info(t['premium_msg'])
        st.stop()
    ...
    # Four Pillars Compact (ikon unicode)
    st.markdown("""
    ...
    if simulate_clicked:
        ...
        st.markdown(f'<p class="risk-data-stream">SYS.TIMESTAMP: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} &middot; ENCRYPTION: AES-256 &middot; NODE: AEROVULPIS-PRIME</p>', unsafe_allow_html=True)
...
# ==============================================================================
    ...
    # Render semua kartu pricing dalam SATU HTML string (perbaikan)
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # close upgrade-container

    # =====================================================================
    # PAYMENT MODAL (Muncul jika show_payment_modal == True)
    # =====================================================================
    if st.session_state.show_payment_modal:
        ...
        # --- Generate QR Code ASLI menggunakan library qrcode ---
        ...
        if qr_base64:
            ...
            # Fallback SVG jika qrcode tidak terinstall
            ...
            <div class="payment-modal">
                ...
                <div class="payment-info-grid">
                    <div class="payment-info-item">
                        <div class="payment-info-label">Package</div>
                        <div class="payment-info-value">{pkg_name}</div>
                    </div>
                    <div class="payment-info-item">
                        <div class="payment-info-label">Amount</div>
                        <div class="payment-info-value">{pkg_price}</div>
                    </div>
                    <div class="payment-info-item">
                        <div class="payment-info-label">User ID</div>
                        <div class="payment-info-value">{user_id[:20] if user_id else 'GUEST'}...</div>
                    </div>
                    <div class="payment-info-item">
                        <div class="payment-info-label">Status</div>
                        <div class="payment-info-value" style="color:#ffcc00;">AWAITING PAYMENT</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        with col_btn:
            if st.button("SUBMIT PROOF", key="submit_payment", width='stretch'):
                if st.session_state.payment_proof_uploaded:
                    ...
                    st.info("Once verified, you will receive a LICENSE KEY via Telegram. Activate it in the sidebar menu (ACTIVATE LICENSE KEY).")
                    time.sleep(3)
