...
translations = {
    "ID": {
        ...
        "logging_out": "TERMINATING SECURE SESSION",
...
def get_news_data(category="General", max_articles=10):
    """Fetch news from multiple sources with 1-hour intelligent caching"""
    ...
    if not berita_final:
        cached_news = get_cached_news(category)
        return cached_news, "UNABLE TO FETCH LIVE NEWS | DISPLAYING CACHED DATA"
...
with st.sidebar:
    ...
    if st.session_state.auth_session and st.session_state.user_name:
        ...
        try:
            ...
            st.info("CONFIGURE GOOGLE OAUTH IN SETTINGS")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if error and not articles:
        ...
        for a in articles:
            ...
            <div class="news-card">
                ...
                <p style="font-family:Rajdhani;font-size:11px;color:#8899bb;line-height:1.5;">{a.get('description','')[:280]}{'...' if len(a.get('description',''))>280 else ''}</p>
...
# ====================== 11. HELP & SUPPORT ======================
    ...
    with st.expander("SIGNAL ANALYSIS"):
        ...
        **Indicator Categories:**
        - **Trend:** SMA 50, SMA 200, EMA 9/21, KAMA, Ichimoku A/B, Parabolic SAR
        - **Momentum:** RSI, MACD, Stochastic K, CCI, Williams %R, MFI, ROC, TRIX, AO
    ...
    with st.expander("MARKET SESSIONS & NEWS"):
        ...
        **Global Market Session Monitor:**
        ...
        - Visual progress bars showing session completion percentage
        - Golden Hour detection for London-New York overlap period
