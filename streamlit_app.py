...
# ##############################################################################
# CYBER-TECH CSS (termasuk perbaikan QR dan Upgrade)
# ##############################################################################
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    .pricing-btn { display: inline-block; background: transparent; border: 1px solid rgba(0,212,255,0.4); color: #00d4ff; font-family: 'Orbitron', sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 3px; padding: 9px 22px; border-radius: 2px; text-decoration: none; transition: all 0.3s ease; text-transform: uppercase; width: 100%; cursor: pointer; }
    .pricing-btn:hover { background: rgba(0,212,255,0.08); border-color: #00d4ff; box-shadow: 0 0 25px rgba(0,212,255,0.3); color: #ffffff; }
    .pricing-btn.featured-btn { border-color: #00ff88; color: #00ff88; }
    .pricing-btn.featured-btn:hover { background: rgba(0,255,136,0.08); border-color: #00ff88; box-shadow: 0 0 25px rgba(0,255,136,0.3); color: #00ff88; }
...
# ##############################################################################
# MARKET DATA FUNCTIONS (MULTI-SOURCE: Finnhub -> Cache -> yfinance)
# ##############################################################################

def get_market_data(ticker_symbol):
    """
    Multi-source market data dengan prioritas:
    1. Finnhub API (real-time, 60 calls/min, gratis)
    2. Database Cache dari market_prices (5 detik)
    ...
    try:
        ...
        if finnhub_result and finnhub_result.get("price", 0) > 0:
            ...
            return {
                "price": finnhub_result["price"],
                "change": 0,
                "change_pct": 0,
                "source": finnhub_result.get("source", "FINNHUB"),
                "spread": 0
            }
...
# ##############################################################################
# QR CODE GENERATOR (UNTUK PEMBAYARAN)
# ##############################################################################

def generate_qr_base64(data_string, size=180):
    """Membuat QR code dalam format base64 PNG."""
    try:
        import qrcode
        from io import BytesIO
        ...
        # Fallback ke QR code sederhana via API publik jika library tidak ada
        return None
...
# ##############################################################################
# SIDEBAR CONTROL CENTER (DENGAN PROTEKSI TIER)
# ##############################################################################
...
with st.sidebar:
    ...
    # Tentukan apakah user sudah login dan tiernya
    user_is_premium = st.session_state.auth_session and st.session_state.user_tier != "free"
    
    # Menu Navigasi dengan opsi yang dikunci jika perlu
    menu_options = ["Live Dashboard"]
    menu_icons = ["activity"]
    
    # Tambahkan opsi premium hanya jika user premium
    if user_is_premium:
        menu_options += ["AeroVulpis Sentinel", "Signal Analysis", "Market Sessions", "Market News", "Smart Alert Center"]
        menu_icons += ["shield-shaded", "graph-up-arrow", "globe", "newspaper", "bell-fill"]
    else:
        # Opsi premium dikunci, tetapi tetap ditampilkan dengan ikon kunci
        menu_options += ["AeroVulpis Sentinel (Premium)", "Signal Analysis (Premium)", "Market Sessions (Premium)", "Market News (Premium)", "Smart Alert Center (Premium)"]
        menu_icons += ["lock-fill", "lock-fill", "lock-fill", "lock-fill", "lock-fill"]
    
    menu_options += ["Economic Radar", "Chatbot AI", "Risk Management", "Tingkatkan Level", "Settings", "Help & Support"]
    menu_icons += ["calendar-event", "chat-dots", "shield-fill", "rocket-takeoff", "gear", "question-circle"]
    ...
    with col_chart:
        ...
        if st.button(t['sentinel_btn'], key="sentinel_pro_btn", width='stretch'):
            ...
            if market and not df.empty:
                ...
                # Tampilkan animasi 3D loading yang keren
...
# ==============================================================================
# 9. RISK MANAGEMENT (REVAMP TOTAL - COMPACT CYBER-TECH)
# ==============================================================================
...
# ==============================================================================
# 10. TINGKATKAN LEVEL (UPGRADE PAGE - CYBER-TECH FUTURISTIC)
# ==============================================================================
elif menu_selection == "Tingkatkan Level":
    # --- Session State tetap di sini untuk memastikan modal berfungsi ---
    ...
    if "payment_proof_uploaded" not in st.session_state:
    ...
    <div class="upgrade-container">
        ...
        <p class="upgrade-subtitle">Select Your Plan &middot; Unlock Full Potential &middot; Professional Trading Terminal</p>
    """, unsafe_allow_html=True)

    # --- Pricing Grid (5 Kolom) menggunakan Columns ---
    packages = [
    ...
    # Render semua kartu pricing menggunakan st.columns
    cols = st.columns(5)
    ...
    if st.session_state.show_payment_modal:
        ...
        if qr_base64:
            ...
            # Fallback ke SVG jika qrcode tidak terinstall
        ...
        <div class="payment-modal-overlay" id="payment-modal">
            <div class="payment-modal">
                ...
                <p class="payment-modal-subtitle">Secure QRIS Transaction &middot; Encrypted &middot; Verified</p>
        ...
        with col_btn:
            if st.button("SUBMIT\nPROOF", key="submit_payment", width='stretch'):
...
# ==============================================================================
    ...
    with st.expander("LIVE DASHBOARD"):
        ...
        **Fitur:**
        - Harga live dengan indikator sumber data (FINNHUB / CACHE / LIVE)
        ...
        **Catatan Penting:** Harga yang ditampilkan mungkin memiliki selisih beberapa pip dengan platform trading seperti MetaTrader 5 (MT5). Hal ini disebabkan oleh perbedaan sumber data, spread broker, dan waktu pembaruan. AeroVulpis menggunakan data dari penyedia independen dan tidak terhubung langsung ke broker manapun.
        """)
