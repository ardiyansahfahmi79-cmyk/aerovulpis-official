...
def sync_user_to_supabase(user_id, email, name, avatar=""):
    """Sinkronisasi data user ke tabel users & user_tiers."""
    try:
        supabase = get_supabase_client()
        ...
        if existing.data:
            ...
            # Insert tier free via service_role biar pasti masuk
            try:
                supabase_admin = get_supabase_admin()
...
# ##############################################################################
# CTRADER WEBSOCKET REAL-TIME PRICE
# ##############################################################################
    ...
    def run_websocket():
        ...
        try:
            ...
            try:
                ...
                print("CTRADER: websocket-client not installed! Using REST API fallback.")
                return
            ...
            try:
                ...
                if "symbolId" in data and "bid" in data:
                    ...
                    st.session_state.ctrader_prices[symbol_name] = {
                        "price": mid_price, "bid": bid, "ask": ask,
        ...
        def on_open(ws):
            ...
            sub_msg = {"type": "subscribe", "symbols": subscribe_symbols, "access_token": token}
        ...
        try:
            ws = WebSocketApp("wss://ctraderapi.com/ws", on_open=on_open, on_message=on_message, on_error=on_error, on_close=on_close)
...
def get_icmarket_price(symbol):
        ...
        if updated_at:
            try:
                ...
                if (now - updated_dt).total_seconds() < 5:
                    return {"price": data["price"], "bid": data["bid"], "ask": data["ask"], "spread": data["spread"], "source": "ICMARKET_WS"}
            except Exception:
                pass
    ...
    ctrader_map = {
        ...
        "AUDUSD": "6", "USDCHF": "7", "BTCUSD": "100", "ETHUSD": "101", "SOLUSD": "102",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if symbol in ["XAUUSD", "XAGUSD", "BTCUSD", "ETHUSD"]:
                ...
                formatted_price = round(mid_price, 4)
            spread = round(ask - bid, 5)
            return {"price": formatted_price, "bid": bid, "ask": ask, "spread": spread, "source": "ICMARKET"}
    except Exception:
        pass
    return None
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    /* ==================== RISK MANAGEMENT CYBER-TECH v2.0 ==================== */
    .risk-cyber-container {
        border: 1px solid rgba(0,212,255,0.25);
        border-radius: 8px;
        padding: 32px 28px;
        background: linear-gradient(160deg, rgba(0,12,32,0.95), rgba(0,4,16,0.98));
        box-shadow: 0 0 80px rgba(0,212,255,0.06), inset 0 1px 0 rgba(0,212,255,0.04);
        margin-bottom: 20px;
        position: relative;
        overflow: hidden;
    }
    ...
    .risk-projection-card {
        background: linear-gradient(160deg, rgba(0,28,65,0.8), rgba(0,8,35,0.9));
...
def get_market_data(ticker_symbol):
    ...
    1. cTrader WebSocket/REST API (real-time XAUUSD, XAGUSD, Forex, Crypto)
    2. Database Cache (3 detik freshness dari cTrader)
    3. yfinance (global market data fallback TERAKHIR)
    ...
    try:
        ...
        # 1. cTrader WebSocket/REST (PRIORITAS UTAMA - REAL-TIME)
        ctrader_instruments = [
            "XAUUSD", "XAGUSD",
        ...
        # 2. Database Cache (dari cTrader sebelumnya)
        ...
        # 3. yfinance (FALLBACK TERAKHIR - DELAYED 15-20 MENIT)
...
with st.sidebar:
    ...
    tier_colors = {
        "free": "#556680", "trial": "#00d4ff", "weekly": "#00ff88",
    ...
    tf_options = {
        "15M": {"period": "5d", "interval": "15m"}, "30M": {"period": "5d", "interval": "30m"},
        "1H": {"period": "1mo", "interval": "1h"}, "3H": {"period": "1mo", "interval": "1h"},
        "4H": {"period": "1mo", "interval": "1h"}, "1D": {"period": "1y", "interval": "1d"},
        ...
        with col_g:
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    "bar": {"color": gauge_color, "thickness": 0.25}, "bgcolor": "rgba(0,0,0,0)", "borderwidth": 1, "bordercolor": "rgba(0,212,255,0.3)",
                    "steps": [
                        {"range": [0, 30], "color": "rgba(255,42,109,0.15)"}, {"range": [30, 45], "color": "rgba(255,42,109,0.08)"},
                        {"range": [45, 55], "color": "rgba(255,204,0,0.08)"}, {"range": [55, 70], "color": "rgba(0,255,136,0.08)"},
...
# ==============================================================================
# 9. RISK MANAGEMENT (REVAMP TOTAL - CYBER-TECH FUTURISTIC v2.0)
# ==============================================================================
    ...
    # Four Pillars - CYBER-TECH STYLE (NO EMOJI)
    st.markdown("""
    <div class="risk-pillar-grid">
        <div class="risk-pillar-card">
            <div class="risk-pillar-icon">&#9650;</div>
            <p class="risk-pillar-label">TRADING RULES</p>
            <p class="risk-pillar-desc">SL DEFINITION MATRIX</p>
        </div>
        <div class="risk-pillar-card">
            <div class="risk-pillar-icon">&#9632;</div>
            <p class="risk-pillar-label">POSITION SIZE</p>
            <p class="risk-pillar-desc">QUANTUM SCALE LOGIC</p>
        </div>
        <div class="risk-pillar-card">
            <div class="risk-pillar-icon">&#9670;</div>
            <p class="risk-pillar-label">CONFIDENCE</p>
            <p class="risk-pillar-desc">NEURAL REAL-TIME</p>
        </div>
        <div class="risk-pillar-card">
            <div class="risk-pillar-icon">&#9881;</div>
            <p class="risk-pillar-label">RISK MGMT</p>
            <p class="risk-pillar-desc">TACTICAL PROTOCOL</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_bal:
        st.markdown('<div class="risk-input-card">', unsafe_allow_html=True)
        st.markdown(f'<p class="risk-input-label">{t["account_balance"]}</p>', unsafe_allow_html=True)
        balance = st.number_input("balance_input", value=1000.0, step=100.0, min_value=100.0, key="sim_balance", label_visibility="collapsed")
        st.markdown(f'<p style="font-family:Share Tech Mono;font-size:24px;color:#00ff88;text-shadow:0 0 15px rgba(0,255,136,0.4);margin:0;text-align:center;">${balance:,.2f}</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    wc, lc = st.columns(2)
    with wc:
        st.markdown('<div class="risk-input-card" style="border-color:rgba(0,255,136,0.25);">', unsafe_allow_html=True)
        st.markdown(f'<p class="risk-input-label" style="color:#00ff88;">{t["wins"]}</p>', unsafe_allow_html=True)
        wins = st.number_input("wins_input", min_value=0, value=3, step=1, key="wins", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)
    with lc:
        st.markdown('<div class="risk-input-card" style="border-color:rgba(255,42,109,0.25);">', unsafe_allow_html=True)
        st.markdown(f'<p class="risk-input-label" style="color:#ff2a6d;">{t["losses"]}</p>', unsafe_allow_html=True)
        losses = st.number_input("losses_input", min_value=0, value=2, step=1, key="losses", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    d1, d2 = st.columns(2)
    with d1:
        st.markdown('<div class="risk-input-card" style="border-color:rgba(255,42,109,0.35);">', unsafe_allow_html=True)
        st.markdown('<p class="risk-input-label" style="color:#ff2a6d;">MAX LOSS %</p>', unsafe_allow_html=True)
        max_loss = st.number_input("maxl_input", 1.0, 100.0, 5.0, 1.0, key="maxl", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)
    with d2:
        st.markdown('<div class="risk-input-card" style="border-color:rgba(0,255,136,0.35);">', unsafe_allow_html=True)
        st.markdown('<p class="risk-input-label" style="color:#00ff88;">MAX PROFIT %</p>', unsafe_allow_html=True)
        max_profit = st.number_input("maxp_input", 1.0, 200.0, 10.0, 1.0, key="maxp", label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if simulate_clicked:
        ...
        st.markdown(f"""
        <p style="font-family:Orbitron;font-size:16px;color:#00d4ff;text-align:center;letter-spacing:5px;margin:20px 0;text-shadow:0 0 30px rgba(0,212,255,0.5);">
            {t['projection_title']}
        </p>
        """, unsafe_allow_html=True)
        
        periods = [
            (t['risk_weekly'], wn, wr, fbw, "rgba(0,255,136,0.15)"),
            (t['risk_monthly'], wn * 4, mr, fbm, "rgba(0,212,255,0.15)"),
            (t['risk_yearly'], wn * 52, yr, fby, "rgba(188,19,254,0.15)")
        ]
        ...
        for pn, net, ret, fbal, glow_color in periods:
            ...
            <div class="risk-projection-card" style="box-shadow:0 0 30px {glow_color};">
                <p style="font-family:Orbitron;font-size:12px;color:#00d4ff;margin:0 0 12px;letter-spacing:3px;text-align:center;">{pn}</p>
                <div class="risk-matrix-grid">
                    <div class="risk-matrix-item" style="border-color:{net_color}40;">
                        <div class="risk-matrix-label">{t['risk_net']}</div>
                        <div class="risk-matrix-value" style="color:{net_color};">{net:+,.2f}</div>
                    </div>
                    <div class="risk-matrix-item" style="border-color:{ret_color}40;">
                        <div class="risk-matrix-label">{t['risk_return']}</div>
                        <div class="risk-matrix-value" style="color:{ret_color};">{ret:+.1f}%</div>
                    </div>
                    <div class="risk-matrix-item">
                        <div class="risk-matrix-label">{t['risk_balance']}</div>
                        <div class="risk-matrix-value">{fbal:,.2f}</div>
                    </div>
                    <div class="risk-matrix-item">
                        <div class="risk-matrix-label">CONFIDENCE</div>
                        <div class="risk-matrix-value" style="color:#bc13fe;">{min(99,abs(int(ret)))}%</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div class="risk-projection-card warning">
            <div class="risk-matrix-grid">
                <div class="risk-matrix-item" style="border-color:#ff2a6d40;">
                    <div class="risk-matrix-label">{t['risk_per_trade']}</div>
                    <div class="risk-matrix-value" style="color:#ff2a6d;">{ra:,.2f}</div>
                </div>
                <div class="risk-matrix-item" style="border-color:#00ff8840;">
                    <div class="risk-matrix-label">{t['risk_reward_trade']}</div>
                    <div class="risk-matrix-value" style="color:#00ff88;">{rw:,.2f}</div>
                </div>
                <div class="risk-matrix-item" style="border-color:#ff2a6d40;">
                    <div class="risk-matrix-label">{t['risk_max_loss']}</div>
                    <div class="risk-matrix-value" style="color:#ff2a6d;">-{mla:,.2f}</div>
                </div>
                <div class="risk-matrix-item" style="border-color:#00ff8840;">
                    <div class="risk-matrix-label">{t['risk_max_profit']}</div>
                    <div class="risk-matrix-value" style="color:#00ff88;">+{mpa:,.2f}</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="risk-projection-card" style="border:1px solid #00d4ff;box-shadow:0 0 40px rgba(0,212,255,0.2);">
            <div class="risk-matrix-grid">
                <div class="risk-matrix-item">
                    <div class="risk-matrix-label">{t['risk_initial']}</div>
                    <div class="risk-matrix-value">{balance:,.2f}</div>
                </div>
                <div class="risk-matrix-item" style="border-color:{'#00ff88' if fbw>=balance else '#ff2a6d'}40;">
                    <div class="risk-matrix-label">{t['risk_after']} 1W</div>
                    <div class="risk-matrix-value" style="color:{'#00ff88' if fbw>=balance else '#ff2a6d'};">{fbw:,.2f}</div>
                </div>
                <div class="risk-matrix-item" style="border-color:{'#00ff88' if fbm>=balance else '#ff2a6d'}40;">
                    <div class="risk-matrix-label">{t['risk_after']} 1M</div>
                    <div class="risk-matrix-value" style="color:{'#00ff88' if fbm>=balance else '#ff2a6d'};">{fbm:,.2f}</div>
                </div>
                <div class="risk-matrix-item" style="border-color:{'#00ff88' if fby>=balance else '#ff2a6d'}40;">
                    <div class="risk-matrix-label">{t['risk_after']} 1Y</div>
                    <div class="risk-matrix-value" style="color:{'#00ff88' if fby>=balance else '#ff2a6d'};">{fby:,.2f}</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <p class="risk-data-stream">
            SYS.TIMESTAMP: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} &middot; ENCRYPTION: AES-256 &middot; NODE: AEROVULPIS-PRIME &middot; STATUS: NOMINAL
        </p>
        """, unsafe_allow_html=True)
