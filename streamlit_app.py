...
# ----- FUNGSI AUTH (DIPERBAIKI) -----

def delete_user_with_verification(email, password):
    """
    Verifikasi kredensial user sebelum menghapus akun.
    Menghapus data di tabel public terlebih dahulu menggunakan admin client
    ...
    try:
        ...
        # 2. Hapus dari tabel kustom TERLEBIH DAHULU menggunakan admin client
        supabase_admin.table("user_tiers").delete().eq("user_id", user_id).execute()
        supabase_admin.table("users").delete().eq("id", user_id).execute()

        # 3. Terakhir, hapus dari auth.users
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        # Form Lupa Password (DIPERBAIKI - tambah redirect_to)
                ...
                with col_r1:
                    if st.button("Kirim Link Reset", key="send_reset_btn", width='stretch'):
                        if reset_email:
                            try:
                                supabase_auth = get_supabase_client()
                                redirect_url = "http://localhost:8501"  # Ganti dengan URL production nanti
        ...
        # Form Delete Account (DIPERBAIKI - urutan hapus)
