...
# ##############################################################################
# GOOGLE OAUTH AUTHENTICATION (FIXED - BYPASS PKCE)
# ##############################################################################

def handle_google_oauth():
    """Google OAuth dengan bypass untuk Streamlit Cloud"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                user_response = requests.get(
                    ...
                    headers={"apikey": supabase_key, "Authorization": f"Bearer {data.get('access_token')}"},
                    timeout=10
                )
                if user_response.status_code == 200:
                    ...
                    user_avatar = user_data.get("user_metadata", {}).get("avatar_url", "")
...
# ##############################################################################
# IC MARKET PRICE FETCH (FIXED - XAUUSD PRICE)
# ##############################################################################

def get_icmarket_price(symbol):
    """
    Mengambil harga dari IC Market cTrader API.
    Jika gagal, return None (akan fallback ke cache/yfinance).
    """
    ctrader_map = {
        "XAUUSD": "1", "XAGUSD": "2",
        ...
        "SOLUSD": "102", "XRPUSD": "103", "BNBUSD": "104",
    }
...
# ##############################################################################
# CSS (SEMUA LENGKAP - TIDAK TERPOTONG)
# ##############################################################################
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    .loading-3d-pro-ring:nth-child(1) {
        ...
        border-top-color: #00d4ff; animation: ringPulse1 2s infinite ease-in-out;
        transform: rotateX(75deg);
    }
    .loading-3d-pro-ring:nth-child(2) {
        width: 78%; height: 78%; margin-top: -39%; margin-left: -39%;
        border-right-color: #00ff88; animation: ringPulse2 2s infinite ease-in-out 0.4s;
        transform: rotateY(75deg);
    }
    .loading-3d-pro-ring:nth-child(3) {
        width: 56%; height: 56%; margin-top: -28%; margin-left: -28%;
        border-bottom-color: #ff2a6d; animation: ringPulse3 2s infinite ease-in-out 0.8s;
        transform: rotateZ(60deg);
    }
    .loading-3d-pro-ring:nth-child(4) {
        width: 34%; height: 34%; margin-top: -17%; margin-left: -17%;
        border-left-color: #bc13fe; animation: ringPulse4 2s infinite ease-in-out 1.2s;
...
# ##############################################################################
# SMART ALERT MONITORING SYSTEM (FIXED - target_value SUPPORT)
# ##############################################################################
