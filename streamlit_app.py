...
# ##############################################################################
# DELETE USER BY CREDENTIALS (LOGIN DULU, LALU HAPUS)
# ##############################################################################

def delete_user_by_credentials(email, password):
    """
    Login dengan kredensial user, dapatkan user_id, lalu hapus dari auth.users dan tabel kustom.
    """
    try:
        supabase = get_supabase_client()
        # Login sebagai user
        ...
        if resp and resp.user:
            ...
            return False, "Gagal login. Pastikan email dan password benar."
    except Exception as e:
        error_msg = str(e)
        if "Invalid login credentials" in error_msg:
            return False, "Email atau password salah. Tidak dapat menghapus akun."
        return False, f"Gagal menghapus user: {error_msg}"
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        if st.session_state.get("show_delete_account", False):
            with st.container():
                st.warning("PERHATIAN: Ini akan menghapus akun Anda secara permanen. Semua data (tier, aktivitas, cache) akan hilang. Masukkan email dan password akun yang akan dihapus.")
                ...
                with col_d1:
                    if st.button("HAPUS AKUN", key="confirm_delete_btn", width='stretch', type="primary"):
                        if del_email and del_pass:
                            success, msg = delete_user_by_credentials(del_email.strip(), del_pass)
                            if success:
                                st.success(msg)
