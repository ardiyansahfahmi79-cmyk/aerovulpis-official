...
# ##############################################################################
# CYBER-TECH CSS (TERMASUK UPGRADE & PAYMENT MODAL 300px)
# ##############################################################################
            ...
            if login_submitted and email_input and password_input:
                try:
                    ...
                    if "Email not confirmed" in error_msg:
                        ...
                        st.error("Email atau password salah. Jika Anda yakin password benar, gunakan reset password di bawah atau hubungi admin.")
                    else:
                ...
                if len(password_input) < 6:
                    ...
                    try:
                        ...
                        if resp and resp.user:
                            ...
                            st.info("Jika email sudah terdaftar, silakan login atau hapus akun di Authentication > Users di dashboard Supabase.")
                    except Exception as e:
                        err = str(e)
                        if "already registered" in err.lower():
                            st.warning("Email sudah terdaftar di sistem. Untuk mendaftar ulang, Anda harus menghapus pengguna di dashboard Supabase: Authentication > Users. Atau gunakan fitur lupa password.")
                        else:
                            st.error(f"REGISTRATION FAILED: {err}")
        st.markdown('</div>', unsafe_allow_html=True)

        # Fitur Lupa Password di luar form
        st.markdown("---")
        if st.button("LUPA PASSWORD / RESET", key="forgot_password_btn"):
            st.session_state.show_forgot_password = True

        if st.session_state.get("show_forgot_password", False):
            reset_email = st.text_input("Masukkan email untuk reset password", key="reset_email", placeholder="EMAIL ADDRESS")
            if st.button("Kirim Link Reset Password", key="send_reset_btn"):
                try:
                    supabase_auth = get_supabase_client()
