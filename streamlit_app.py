# ##############################################################################
# PART 1: SETUP, CSS, SIDEBAR, FUNCTIONS (FINAL WITH LIMITS)
# ##############################################################################
...
def activate_key(user_id, key_code):
    ...
    try:
        ...
        check_tier = supabase_admin.table("user_tiers").select("*").eq("user_id", user_id).execute()
        tier_data = {
            "tier": tier,
        ...
        if check_tier.data:
            supabase_admin.table("user_tiers").update(tier_data).eq("user_id", user_id).execute()
        else:
            tier_data["user_id"] = user_id
            supabase_admin.table("user_tiers").insert(tier_data).execute()
...
def delete_user_with_verification(email, password):
    try:
        supabase = get_supabase_client()
        resp = supabase.auth.sign_in_with_password({"email": email.strip(), "password": password})
        if not resp or not resp.user:
            return False, "[AUTH] Verifikasi gagal. Pastikan email dan password benar."
        user_id = resp.user.id
        supabase_admin = get_supabase_admin()
        supabase_admin.table("user_tiers").delete().eq("user_id", user_id).execute()
        supabase_admin.table("users").delete().eq("id", user_id).execute()
        supabase_admin.auth.admin.delete_user(user_id)
        return True, "[SYSTEM] Akun berhasil dihapus permanen."
    except Exception as e:
        error_msg = str(e).lower()
        if "invalid login credentials" in error_msg or "invalid_grant" in error_msg:
            return False, "[AUTH] Email atau password salah. Tidak dapat menghapus akun."
...
def get_user_usage(user_id):
    ...
    try:
        supabase = get_supabase_client()
        res = supabase.table("users").select("analysis_count, chatbot_count, sentinel_count, last_reset_date").eq("id", user_id).execute()
...
def increment_user_usage(user_id, usage_type):
    ...
    try:
        ...
        res = supabase.table("users").select(column).eq("id", user_id).execute()
        if res.data and len(res.data) > 0:
            current_val = res.data[0].get(column, 0) or 0
            supabase.table("users").update({column: current_val + 1}).eq("id", user_id).execute()
    except Exception:
        pass
...
translations = {
    "ID": {
        ...
        "alert_title": "SMART ALERT CENTER", "alert_subtitle": "AEROVULPIS TERMINAL V3.6",
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    .loading-3d-pro-container { text-align: center; padding: 20px; }
    .loading-3d-pro-scene { position: relative; display: inline-block; width: 120px; height: 120px; }
    .loading-3d-pro-core { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); }
    .loading-3d-pro-ring { position: absolute; top: 50%; left: 50%; border-radius: 50%; border: 2px solid rgba(0, 212, 255, 0.6); animation: ringSpin 2s linear infinite; transform-origin: 0 0; }
    .loading-3d-pro-ring:nth-child(1) { width: 30px; height: 30px; margin-left: -15px; margin-top: -15px; animation-delay: 0s; }
    .loading-3d-pro-ring:nth-child(2) { width: 45px; height: 45px; margin-left: -22.5px; margin-top: -22.5px; animation-delay: 0.2s; }
    .loading-3d-pro-ring:nth-child(3) { width: 60px; height: 60px; margin-left: -30px; margin-top: -30px; animation-delay: 0.4s; }
    .loading-3d-pro-ring:nth-child(4) { width: 75px; height: 75px; margin-left: -37.5px; margin-top: -37.5px; animation-delay: 0.6s; }
    @keyframes ringSpin { 0% { transform: rotateX(0deg) rotateY(0deg) rotateZ(0deg); opacity: 0.8; } 50% { opacity: 0.3; } 100% { transform: rotateX(360deg) rotateY(180deg) rotateZ(360deg); opacity: 0.8; } }
    .loading-3d-pro-center { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 12px; height: 12px; background: #00d4ff; border-radius: 50%; box-shadow: 0 0 10px #00d4ff; animation: centerPulse 1.5s ease-in-out infinite; }
    @keyframes centerPulse { 0%, 100% { transform: translate(-50%, -50%) scale(1); } 50% { transform: translate(-50%, -50%) scale(1.4); } }
    .loading-3d-pro-text { font-family: 'Orbitron', sans-serif; font-size: 14px; color: #00d4ff; text-transform: uppercase; letter-spacing: 3px; margin-top: 15px; animation: textGlow 2s ease-in-out infinite; }
    @keyframes textGlow { 0%, 100% { text-shadow: 0 0 8px #00d4ff; } 50% { text-shadow: 0 0 20px #00d4ff, 0 0 30px #00ff88; } }
    .loading-3d-pro-sub { font-family: 'Share Tech Mono', monospace; font-size: 10px; color: #557799; margin-top: 5px; }
</style>
...
# Market data functions (TIDAK BERUBAH)
...
def get_groq_response(question, context=""):
    ...
    system_prompt = f"""AEROVULPIS NEURAL SYSTEM V3.6
...
def get_sentinel_analysis(asset_name, market_data, df, signal, reasons):
    ...
    if user_limits["sentinel_per_day"] == 0:
        return "SENTINEL PRO ACCESS RESTRICTED | UPGRADE TIER OR CONTACT ADMIN"
...
<div class="main-title-container">
    ...
    <p class="subtitle-text">V3.6 ULTIMATE</p>
</div>
""", unsafe_allow_html=True)

# ##############################################################################
# SIDEBAR CONTROL CENTER (FINAL - AUTH FIX + EXPIRED DATE + USAGE LIMITS)
# ##############################################################################
...
with st.sidebar:
    ...
    if not st.session_state.auth_session:
        ...
        with st.form("email_password_login_form"):
            ...
            if login_submitted and email_input and password_input:
                try:
                    ...
                    error_msg = str(e).lower()
                    if "invalid login credentials" in error_msg or "invalid_grant" in error_msg:
                        st.error("[AUTH] Email atau password salah.")
                    elif "email not confirmed" in error_msg:
                        st.error("[AUTH] Email belum dikonfirmasi. Silakan cek inbox/spam Anda.")
                    else:
            ...
            if signup_submitted and email_input and password_input:
                ...
                if len(password_input) < 6:
                    st.error("[AUTH] PASSWORD MUST BE AT LEAST 6 CHARACTERS")
                else:
                    try:
                        supabase_auth = get_supabase_client()
                        ...
                        if resp and resp.user:
                            ...
                            st.session_state.user_name = resp.user.user_metadata.get("full_name") or (email_clean.split("@")[0] if email_clean else "USER")
                            st.session_state.user_email = resp.user.email or ""
                            st.session_state.user_avatar = ""
                            tier, exp = get_user_tier(resp.user.id)
                            st.session_state.user_tier = tier
                            st.session_state.user_expired_at = exp[:10] if exp else None
                        ...
                        if "already registered" in err or "already exists" in err:
                            st.warning("[SYSTEM] Email sudah terdaftar. Silakan LOGIN atau gunakan fitur LUPA PASSWORD.")
                        else:
                    ...
                    if st.button("Kirim Link Reset", key="send_reset_btn", width='stretch'):
                        if reset_email:
                            try:
                                ...
                                st.success(f"[SYSTEM] Link reset password telah dikirim ke {reset_email}.")
                                st.session_state.show_forgot_password = False
                            except Exception as e:
                                st.error(f"[SYSTEM] Gagal: {str(e)}")
                        else:
                            st.warning("[SYSTEM] Masukkan email terlebih dahulu.")
                with col_r2:
        ...
        if st.session_state.get("show_delete_account", False):
            with st.container():
                st.warning("[SYSTEM] PERHATIAN: Akun akan dihapus permanen. Verifikasi diperlukan untuk keamanan.")
                del_email = st.text_input("Email akun yang akan dihapus", key="del_email", placeholder="Masukkan email")
                del_pass = st.text_input("Password akun", key="del_pass", type="password", placeholder="Masukkan password untuk verifikasi")
                col_d1, col_d2 = st.columns(2)
                with col_d1:
                    if st.button("HAPUS PERMANEN", key="confirm_delete_btn", width='stretch', type="primary"):
                        if del_email and del_pass:
                            success, msg = delete_user_with_verification(del_email.strip(), del_pass)
                            if success:
                                st.success(msg)
                            ...
                            st.warning("[SYSTEM] Masukkan email dan password untuk verifikasi.")
                with col_d2:
    ...
    st.markdown("<p style='font-family:Share Tech Mono;font-size:9px;color:#445566;text-align:center;margin:8px 0;'>AEROVULPIS V3.6 | DYNAMIHATCH</p>", unsafe_allow_html=True)
    ...
    <div style="background:rgba(0,15,30,0.5);border:1px solid rgba(0,212,255,0.1);border-radius:4px;padding:12px;margin-top:8px;">
        ...
        <div style="margin-bottom:6px;">
            <p style="font-family:Share Tech Mono;font-size:10px;color:#00d4ff;margin:2px 0;display:flex;justify-content:space-between;">
                <span>AI DEEP ANALYSIS</span><span>{st.session_state.daily_analysis_count}/{user_limits['analysis_per_day']}</span>
            </p>
            <div style="background:rgba(255,255,255,0.05);height:3px;border-radius:1px;overflow:hidden;">
                <div style="background:#00d4ff;width:{min(100,(st.session_state.daily_analysis_count/user_limits['analysis_per_day'])*100) if user_limits['analysis_per_day'] > 0 else 0}%;height:100%;border-radius:1px;"></div>
            </div>
        </div>
        <div style="margin-bottom:6px;">
            <p style="font-family:Share Tech Mono;font-size:10px;color:#bc13fe;margin:2px 0;display:flex;justify-content:space-between;">
                <span>AI SENTINEL PRO</span><span>{st.session_state.daily_sentinel_count}/{user_limits['sentinel_per_day']}</span>
            </p>
            <div style="background:rgba(255,255,255,0.05);height:3px;border-radius:1px;overflow:hidden;">
                <div style="background:#bc13fe;width:{min(100,(st.session_state.daily_sentinel_count/user_limits['sentinel_per_day'])*100) if user_limits['sentinel_per_day'] > 0 else 0}%;height:100%;border-radius:1px;"></div>
            </div>
        </div>
        <div style="margin-bottom:6px;">
    ...
    """, unsafe_allow_html=True)
    # ##############################################################################
# PART 2: MAIN APPLICATION LOGIC & FOOTER
# ##############################################################################
...
if menu_selection == "AeroVulpis Sentinel" or menu_selection == "AeroVulpis Sentinel (Premium)":
    ...
    with col_intel:
        st.markdown(
            f"""<div class="intelligence-panel"><div class="intel-header">{t['sentinel_intel']}</div><div class="intel-content">""",
            unsafe_allow_html=True)
...
# ==============================================================================
    ...
    if market and not df.empty:
        ...
        with c1:
            st.markdown(
                f'<div class="glass-card"><p style="color:#557799;margin:0;font-size:9px;letter-spacing:2px;">{t["live_price"]} [{data_source}]</p><p style="font-family:Share Tech Mono;color:#00ff88;font-size:24px;margin:0;text-shadow:0 0 10px rgba(0,255,136,0.4);">{formatted_price}</p></div>',
                unsafe_allow_html=True)
        with c2:
            color = "#00ff88" if "BUY" in signal else "#ff2a6d" if "SELL" in signal else "#ffcc00"
            st.markdown(
                f'<div class="glass-card"><p style="color:#557799;margin:0;font-size:9px;letter-spacing:2px;">{t["signal"]}</p><p style="font-family:Orbitron;font-size:20px;margin:0;color:{color};text-shadow:0 0 15px {color};">{signal}</p></div>',
                unsafe_allow_html=True)
        with c3:
            rsi_val = df["RSI"].iloc[-1] if "RSI" in df.columns else 0.0
            st.markdown(
                f'<div class="glass-card"><p style="color:#557799;margin:0;font-size:9px;letter-spacing:2px;">{t["rsi"]}</p><p style="font-family:Share Tech Mono;color:#00d4ff;font-size:24px;margin:0;">{rsi_val:.1f}</p></div>',
                unsafe_allow_html=True)
        with c4:
            atr_val = df["ATR"].iloc[-1] if "ATR" in df.columns else 0.0
            st.markdown(
                f'<div class="glass-card"><p style="color:#557799;margin:0;font-size:9px;letter-spacing:2px;">{t["atr"]}</p><p style="font-family:Share Tech Mono;color:#8899bb;font-size:24px;margin:0;">{atr_val:.4f}</p></div>',
                unsafe_allow_html=True)
        ...
        with col_g:
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge={"axis": {"range": [0, 100], "tickwidth": 1, "tickcolor": "#00d4ff", "tickfont": {"color": "#8899bb", "size": 10}},
                       "bar": {"color": gauge_color, "thickness": 0.25}, "bgcolor": "rgba(0,0,0,0)", "borderwidth": 1,
                       "bordercolor": "rgba(0,212,255,0.3)",
                       "steps": [{"range": [0, 30], "color": "rgba(255,42,109,0.15)"},
                       ...
                       "threshold": {"line": {"color": gauge_color, "width": 3}, "thickness": 0.8, "value": score}},
                delta={"reference": 50, "increasing": {"color": "#00ff88"}, "decreasing": {"color": "#ff2a6d"}, "font": {"size": 12}}
            ))
            fig_gauge.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font={"color": "#8899bb"},
        ...
        with col_a:
            st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
            st.markdown(
                f"<p style='font-family:Orbitron;font-size:13px;color:#00d4ff;letter-spacing:2px;margin-bottom:10px;'>{t['ai_analysis']}</p>",
                unsafe_allow_html=True)
            for r in reasons:
                st.markdown(
                    f"<p style='font-family:Share Tech Mono;font-size:10px;color:#8899bb;margin:3px 0;'>[ {r} ]</p>",
                    unsafe_allow_html=True)
...
# ==============================================================================
    ...
    if not df.empty:
        ...
        st.markdown(
            f"<p style='font-family:Orbitron;font-size:16px;color:#8899bb;letter-spacing:2px;'>{t['recommendation']}: <span style='color:{sig_color};'>{signal}</span></p>",
            unsafe_allow_html=True)

        c1, c2, c3 = st.columns(3)
        c1.markdown(
            f'<div style="text-align:center;background:rgba(0,255,136,0.04);padding:12px;border-radius:4px;border:1px solid rgba(0,255,136,0.15);"><p style="color:#00ff88;font-size:10px;margin:0;letter-spacing:2px;">BULLISH</p><p style="font-family:Orbitron;font-size:26px;margin:0;color:#00ff88;">{bull}</p></div>',
            unsafe_allow_html=True)
        c2.markdown(
            f'<div style="text-align:center;background:rgba(255,42,109,0.04);padding:12px;border-radius:4px;border:1px solid rgba(255,42,109,0.15);"><p style="color:#ff2a6d;font-size:10px;margin:0;letter-spacing:2px;">BEARISH</p><p style="font-family:Orbitron;font-size:26px;margin:0;color:#ff2a6d;">{bear}</p></div>',
            unsafe_allow_html=True)
        c3.markdown(
            f'<div style="text-align:center;background:rgba(255,204,0,0.04);padding:12px;border-radius:4px;border:1px solid rgba(255,204,0,0.15);"><p style="color:#ffcc00;font-size:10px;margin:0;letter-spacing:2px;">NEUTRAL</p><p style="font-family:Orbitron;font-size:26px;margin:0;color:#ffcc00;">{neut}</p></div>',
            unsafe_allow_html=True)
        ...
        for name, val, sig in indicators_data:
            if "BULLISH" in sig or "STRONG" in sig or "OVERSOLD" in sig:
                ...
                sig_col = "#ffcc00"
            st.markdown(
                f'<div class="indicator-box"><div class="indicator-name">{name}</div><div class="indicator-value">{val}</div><div class="indicator-signal" style="color:{sig_col};">{sig}</div></div>',
                unsafe_allow_html=True)
        st.markdown('</div></div>', unsafe_allow_html=True)
...
# ==============================================================================
# 6. SMART ALERT CENTER (PREMIUM) - DENGAN LIMIT
# ==============================================================================
...
# ==============================================================================
# 8. ECONOMIC RADAR (TERBUKA SEMUA TIER)
# ==============================================================================
...
# ==============================================================================
# 9. CHATBOT AI (TERBUKA SEMUA TIER)
# ==============================================================================
...
# ==============================================================================
# 10. TINGKATKAN LEVEL (UPGRADE PAGE) - PERBAIKAN TOTAL HTML
# ==============================================================================
...
<p style="font-family:'Share Tech Mono'; font-size:10px; color:#557799; margin-bottom:15px;">Secure QRIS Transaction · Encrypted</p>
''' + f'''
...
<p style="font-size:12px; color:#8899bb; text-align:left;"><b>FORMAT PESAN TELEGRAM:</b> (Klik ikon copy di pojok kanan atas box)</p>
</div>
''', unsafe_allow_html=True)
...
# ==============================================================================
    ...
    with st.expander("AKTIVASI LISENSI & LEVEL"):
        ...
        **Cara Masuk:** Masukkan email dan password di sidebar -> klik "LOGIN" untuk login, atau "REGISTER NEW IDENTITY" untuk daftar.
        **Aktivasi Lisensi:**
        1. Setelah masuk, klik "ACTIVATE LICENSE KEY"
        2. Masukkan kunci lisensi (format: XXXX-XXXX-XXXX-XXXX)
        3. Klik "VALIDATE & ACTIVATE"
        **Level & Batas Harian:**
        - **GRATIS:** 5 Deep Analysis, 0 Sentinel, 20 Chatbot, 1 Smart Alert
        - **TRIAL:** 10 Deep, 1 Sentinel, 50 Chatbot, 2 Alert
        - **MINGGUAN:** 20 Deep, 3 Sentinel, 100 Chatbot, 3 Alert
        - **BULANAN:** 50 Deep, 5 Sentinel, 200 Chatbot, 5 Alert
        - **6 BULAN:** 100 Deep, 10 Sentinel, 500 Chatbot, 10 Alert
        - **TAHUNAN:** 200 Deep, 30 Sentinel, 1000 Chatbot, 20 Alert
        """)
...
<div style="text-align:center;padding:25px;opacity:0.55;">
    ...
    <p style="font-family:Share Tech Mono;font-size:8px;color:#334455;letter-spacing:2px;margin-top:6px;">
        DYNAMIHATCH IDENTITY | V3.6 ULTIMATE | 2026
    </p>
</div>
""", unsafe_allow_html=True)
