# ##############################################################################
# PART 1: SETUP, CSS, SIDEBAR, FUNCTIONS
# ##############################################################################
...
# import qrcode  # SUDAH TIDAK DIGUNAKAN
...
def ensure_user_exists(user_id, email, name):
    """Memastikan user ada di tabel users sebelum insert ke user_tiers."""
    try:
        supabase = get_supabase_client()
...
def get_user_tier(user_id):
    ...
    try:
        ...
        if res.data:
            ...
            # Jika blm ada tier, buat free tier
            try:
...
def activate_key(user_id, key_code):
    ...
    try:
        # Pastikan user ada di tabel users dulu
        ...
        # Hapus tier lama dulu, lalu insert baru
        try:
...
def sync_user_to_supabase(user_id, email, name, avatar=""):
    try:
        ...
        # Pastikan tier free ada
        tier_check = supabase.table("user_tiers").select("user_id").eq("user_id", user_id).execute()
...
# ##############################################################################
# AUTH SESSION RESTORE (DIPERBAIKI)
# ##############################################################################
...
# ##############################################################################
# CYBER-TECH CSS (DIPERBAIKI - MODAL LEBIH KECIL, RESPONSIF)
# ##############################################################################
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    /* PAYMENT MODAL - DIPERBAIKI LEBIH KECIL */
    ...
    .payment-modal { background: linear-gradient(160deg, rgba(0,15,38,0.98), rgba(0,5,18,0.99)); border: 2px solid rgba(0,212,255,0.3); border-radius: 10px; padding: 25px 20px; max-width: 440px; width: 90%; text-align: center; position: relative; box-shadow: 0 0 80px rgba(0,212,255,0.12); }
    .payment-modal::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 2px; background: linear-gradient(90deg, transparent, #00d4ff, #00ff88, #bc13fe, transparent); animation: scanHorizontal 4s infinite; }
    .payment-modal-title { font-family: 'Orbitron', sans-serif; font-size: 16px; font-weight: 800; color: #00d4ff; letter-spacing: 4px; margin-bottom: 3px; }
    ...
    .close-modal-btn { position: absolute; top: 8px; right: 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); color: #8899bb; font-size: 18px; cursor: pointer; font-family: 'Orbitron', sans-serif; width: 28px; height: 28px; border-radius: 3px; display: flex; align-items: center; justify-content: center; z-index: 10000; }
    .close-modal-btn:hover { color: #ff2a6d; border-color: #ff2a6d; background: rgba(255,42,109,0.1); }
    .qr-container { background: #ffffff; padding: 10px; border-radius: 6px; display: inline-block; margin-bottom: 10px; border: 2px solid rgba(0,212,255,0.3); box-shadow: 0 0 20px rgba(0,212,255,0.15); }
    .payment-info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; margin: 10px 0; text-align: left; }
    ...
    .payment-steps { text-align: left; font-family: 'Rajdhani', sans-serif; font-size: 9px; color: #8899bb; margin: 8px 0; line-height: 1.5; }
    .payment-steps span { color: #00d4ff; font-family: 'Orbitron', sans-serif; font-size: 7px; margin-right: 4px; }
    .dynamihatch-watermark { font-family: 'Orbitron', sans-serif; font-size: 7px; color: rgba(255,255,255,0.03); position: absolute; bottom: 8px; right: 15px; letter-spacing: 3px; }
</style>
...
def get_historical_data(ticker_symbol, period="1mo", interval="1h"):
    try:
        ...
        return df.sort_index().dropna() if not df.empty else pd.DataFrame()
    except:
        return pd.DataFrame()
...
def get_groq_response(question, context=""):
    ...
    try:
        chat_completion = client.chat.completions.create(
            messages=[{"role": "system", "content": f"""AEROVULPIS NEURAL SYSTEM V3.5
TIMESTAMP: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} WIB
LANGUAGE: {st.session_state.lang}
CONTEXT: {context}
PROTOCOL: Provide professional technical trading analysis."""}, {"role": "user", "content": question}],
...
RSI: {latest.get('RSI', 0):.2f} | MACD: {latest.get('MACD', 0):.4f} | ATR: {latest.get('ATR', 0):.4f} | ADX: {latest.get('ADX', 0):.2f}
BB: [{latest.get('BB_Lower', 0):.4f} - {latest.get('BB_Upper', 0):.4f}] | Stoch K: {latest.get('Stoch_K', 0):.2f}
REASONS: {', '.join(reasons)}
NEWS: {news_context}
OUTPUT: [KEY LEVELS] [FUNDAMENTAL INSIGHT] [BULLISH SCENARIO] [BEARISH SCENARIO] [FINAL VERDICT] (max 320 words, Bahasa Indonesia)"""
    ...
    analysis = call_openrouter('nousresearch/hermes-3-llama-3.1-405b', "You are AeroVulpis Sentinel Pro Intelligence.")
    if analysis:
        companion = call_openrouter('qwen/qwen3-next-80b-instruct', "Provide additional technical details.")
        if companion: analysis += "\n\n---\nSENTINEL COMPANION:\n" + companion
    if not analysis:
        for model in ['deepseek/deepseek-chat', 'liquid/lfm-2.5-1.2b-thinking', 'minimax/minimax-01']:
            analysis = call_openrouter(model, "You are AeroVulpis Backup Intelligence.")
            if analysis: break
...
def get_deep_analysis(asset_name, market_data, df, signal, reasons):
    ...
    latest = df.iloc[-1]
    price = market_data['price']
    technical_data = f"""INSTRUMENT: {asset_name} | PRICE: {price:,.4f} | SIGNAL: {signal}
...
Stoch K: {latest.get('Stoch_K',0):.2f} | Volume: {df['Volume'].iloc[-1]:,.0f}
REASONS: {', '.join(reasons)}"""
    try:
        chat_completion = client.chat.completions.create(
            messages=[{"role": "system", "content": "AEROVULPIS DEEP ANALYSIS ENGINE V3.5. Expert technical analyst. Max 2000 chars."}, {"role": "user", "content": f"DEEP ANALYSIS:\n{technical_data}\nINCLUDE: RSI, Price vs SMA, Entry (2-3), SL based on ATR, TP (min 1:2 RR), Bullish/Bearish scenarios."}],
...
def market_session_status():
    ...
    for sess in sessions:
        ...
        st.markdown(f"""<div style="background:rgba(0,18,36,0.5);border:1px solid rgba(0,212,255,0.08);border-radius:4px;padding:18px;margin-bottom:10px;">
            ...
            <div style="background:rgba(255,255,255,0.03);height:4px;border-radius:2px;overflow:hidden;"><div style="background:{sess['color'] if is_active else '#333'};width:{progress if is_active else 0}%;height:100%;border-radius:2px;"></div></div>
    ...
    if "ASIAN SESSION" in active_sessions and len(active_sessions) == 1: strategy_text, strategy_detail = "RANGE TRADING PROTOCOL", "Focus on liquidity sweeps and Asian range breakout patterns"
    elif is_golden: strategy_text, strategy_detail = "HIGH VOLATILITY PROTOCOL", "Order block mitigations and FVG entries. Tight spreads, maximum momentum"
    elif "EUROPEAN SESSION" in active_sessions: strategy_text, strategy_detail = "TREND FOLLOWING PROTOCOL", "London breakout patterns. Monitor displacement moves for entry confirmation"
    elif "AMERICAN SESSION" in active_sessions: strategy_text, strategy_detail = "REVERSAL PROTOCOL", "NY open manipulation watch. Late session reversals probability elevated"
...
def get_news_data(category="General", max_articles=10):
    ...
    category_map = {"Stock": "stocks", "Geopolitics": "geopolitics", "Gold & Silver": "gold,silver,precious metals", "Forex": "forex,currency,central banks", "General": "finance"}
        ...
        try:
            ...
            if res_c.get('news'):
                for item in res_c.get('news', []):
                    if item.get('url') and item['url'] not in urls_terpakai:
                        if category == "Forex" and not any(kw.lower() in item.get('title', '').lower() for kw in forex_keywords): continue
    ...
    if not berita_final:
        cached_news = get_cached_news(category)
        if cached_news: return cached_news, "DISPLAYING CACHED DATA"
        return [], "NO NEWS AVAILABLE"
    berita_final = berita_final[:max_articles]
    tz_wib = pytz.timezone('Asia/Jakarta')
    for b in berita_final:
        try:
            raw_date = str(b.get('publishedAt', '')).replace('Z', '+00:00')
            try: dt_utc = datetime.fromisoformat(raw_date)
            except: dt_utc = datetime.strptime(raw_date[:19], "%Y-%m-%dT%H:%M:%S").replace(tzinfo=pytz.UTC)
            b['publishedAt'] = dt_utc.astimezone(tz_wib).strftime("%Y-%m-%d %H:%M WIB")
...
def check_smart_alerts():
    if "active_alerts" not in st.session_state or not st.session_state.active_alerts: return
    ...
    for cat in instruments.values():
        for name, ticker in cat.items(): instrument_to_ticker[name] = ticker
    current_prices = {}
    ...
    for alert in st.session_state.active_alerts:
        if not alert.get("triggered", False):
            inst_name, target_raw, target_value, condition = alert.get("instrument"), alert.get("target"), alert.get("target_value"), alert.get("condition")
            current_price = current_prices.get(inst_name)
            if current_price is None: continue
            try: target_num = float(target_value if target_value else str(target_raw).replace(",", ""))
            except: target_num = 0.0
            if (condition == "bullish" and current_price >= target_num) or (condition == "bearish" and current_price <= target_num):
                alert["triggered"] = True
                now_wib = datetime.now(pytz.timezone('Asia/Jakarta')).strftime("%Y-%m-%d %H:%M:%S WIB")
                alert_message = f"/// AEROVULPIS TARGET ACQUIRED ///\nINSTR: {inst_name}\nPRICE: {format_price_display(current_price, inst_name)}\nTARGET: {format_price_display(target_num, inst_name)}\nTIME: {now_wib}"
                try: requests.post(f"https://api.telegram.org/bot{telegram_bot_token}/sendMessage", json={'chat_id': alert.get("chat_id"), 'text': alert_message}, timeout=10)
                except: pass
...
with st.sidebar:
        ...
        with st.form("email_password_login_form"):
            ...
            if login_submitted and email_input and password_input:
                try:
                    ...
                    error_msg = str(e)
                    if "Email not confirmed" in error_msg: st.error("Please confirm your email address first.")
                    elif "Invalid login credentials" in error_msg: st.error("Invalid email or password.")
                    else: st.error(f"AUTH FAILED: {error_msg}")
    ...
    menu_selection = option_menu(
        ...
        styles={"container": {"padding": "5!important"}, "icon": {"color": "#00d4ff", "font-size": "13px"}, "nav-link": {"font-size": "11px", "text-align": "left", "margin": "2px 0", "padding": "10px 12px", "border-radius": "3px", "font-family": "Rajdhani", "font-weight": "500", "letter-spacing": "1px", "background": "rgba(0,212,255,0.015)", "border": "1px solid rgba(0,212,255,0.06)", "transition": "all 0.25s ease"}, "nav-link-selected": {"background": "linear-gradient(160deg,rgba(0,48,96,0.4),rgba(0,28,64,0.6))", "border": "1px solid #00d4ff", "color": "#00d4ff", "box-shadow": "0 0 18px rgba(0,212,255,0.12)", "font-weight": "700"}}
    )
    ...
    <div style="background:rgba(0,15,30,0.5);border:1px solid rgba(0,212,255,0.1);border-radius:4px;padding:12px;margin-top:8px;">
        ...
        <div style="margin-bottom:6px;">
            <p style="font-family:Share Tech Mono;font-size:10px;color:#00d4ff;margin:2px 0;display:flex;justify-content:space-between;"><span>AI ANALYSIS</span><span>{st.session_state.daily_analysis_count}/{user_limits['analysis_per_day']}</span></p>
            <div style="background:rgba(255,255,255,0.05);height:3px;border-radius:1px;overflow:hidden;"><div style="background:#00d4ff;width:{min(100,(st.session_state.daily_analysis_count/user_limits['analysis_per_day'])*100)}%;height:100%;border-radius:1px;"></div></div>
        </div>
        <div>
            <p style="font-family:Share Tech Mono;font-size:10px;color:#00ff88;margin:2px 0;display:flex;justify-content:space-between;"><span>CHATBOT</span><span>{st.session_state.daily_chatbot_count}/{user_limits['chatbot_per_day']}</span></p>
            <div style="background:rgba(255,255,255,0.05);height:3px;border-radius:1px;overflow:hidden;"><div style="background:#00ff88;width:{min(100,(st.session_state.daily_chatbot_count/user_limits['chatbot_per_day'])*100)}%;height:100%;border-radius:1px;"></div></div>
        </div>
    </div>
    """.replace("{min(100,(st.session_state.daily_analysis_count/user_limits['analysis_per_day'])*100)}", str(min(100, int((st.session_state.daily_analysis_count / user_limits['analysis_per_day']) * 100)))).replace("{min(100,(st.session_state.daily_chatbot_count/user_limits['chatbot_per_day'])*100)}", str(min(100, int((st.session_state.daily_chatbot_count / user_limits['chatbot_per_day']) * 100)))), unsafe_allow_html=True)