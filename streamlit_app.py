...
def get_supabase_client():
    ...
    Digunakan untuk operasi yang memerlukan Row Level Security.
    """
...
def cache_market_price(symbol, price, change_pct=0.0):
    ...
    Menggunakan upsert agar data selalu update.
    """
    try:
        supabase = get_supabase_client()
        data = {
...
def get_cached_market_price_full(symbol):
    """
    Mengambil data harga lengkap dari cache untuk Smart Alert System.
...
def get_user_tier(user_id):
    ...
    Return: (tier, expired_at) atau ('free', None).
    """
    if not user_id:
        return "free", None
    try:
...
def activate_key(user_id, key_code):
    ...
    3. Tandai kunci sebagai sudah digunakan
...
def sync_user_to_supabase(user_id, email, name, avatar=""):
    ...
    Insert jika user baru, update last_login jika sudah ada.
    """
    try:
        supabase = get_supabase_client()
...
def cache_ai_analysis(asset_name, timeframe, analysis):
    ...
    Untuk berbagi hasil analisis antar user.
    """
    try:
        supabase = get_supabase_client()
...
def handle_google_oauth():
    ...
    GOOGLE 403 FIX - Pastikan konfigurasi berikut:
    ...
    4. Supabase Dashboard → Authentication → URL Configuration:
       ...
       - Redirect URL: https://[your-app].streamlit.app
    """
    ...
    if "code" in query_params:
        ...
        try:
            ...
            # FIX: Gunakan exchange_code_for_session dengan parameter auth_code
            ...
            if auth_response and auth_response.user:
                ...
                # Bersihkan URL dan refresh halaman
...
# ====================== ICMARKET PRICE FETCH (AKURAT) ======================
def get_icmarket_price(symbol):
    """
    Mengambil harga real-time dari IC Market via cTrader API.
    
    Format harga per instrumen (sesuai broker):
    - XAUUSD: 2 desimal (contoh: 4756.00)
    - XAGUSD: 2 desimal (contoh: 34.50)
    - EURUSD, GBPUSD, dll: 4 desimal (contoh: 1.0850)
    - BTCUSD, ETHUSD: 2 desimal (contoh: 67250.00)
    
    Return: dict dengan price, bid, ask, spread, source
    """
    ctrader_map = {
        "XAUUSD": "1",
        "XAGUSD": "2",
...
def format_price_display(price, instrument_name):
    ...
    - XAGUSD (Silver):       34.50
    - EURUSD (Forex):      1.0850   (4 digit, strip trailing zeros)
...
# Reset daily limits jika berganti hari
...
def get_market_data(ticker_symbol):
    ...
    try:
        ...
        # 1. IC Market cTrader untuk instrumen yang didukung
        ctrader_instruments = [
            "XAUUSD", "XAGUSD",
...
def add_technical_indicators(df):
    ...
    ATR, ADX, CCI, Williams %R, MFI, TRIX, ROC,
    Awesome Oscillator, KAMA, Ichimoku, Parabolic SAR,
    Volume SMA, Base Line.
    """
    if len(df) < 50:
        return df
...
def get_weighted_signal(df):
    ...
    Menggunakan RSI, MACD, SMA50, SMA200.
...
def get_groq_response(question, context=""):
    """Chatbot AI - AeroVulpis Engine via Groq API"""
    if not client:
...
CONTEXT: {context}
PROTOCOL: Technical trading analysis with entry, stop loss, and take profit levels."""
...
def get_sentinel_analysis(asset_name, market_data, df, signal, reasons):
    """
    AEROVULPIS SENTINEL PRO - Institutional Analysis
    Primary: Hermes 405B + Qwen3 Next 80B
    Backup: Ling-2.6-Flash, LFM2.5-Thinking, Minimax M2.5
    """
...
Language: Indonesian | Max: 320 words | Balanced."""
    ...
    if analysis:
        ...
        if companion:
            analysis += "\n\n---\nSENTINEL COMPANION [QWEN3 NEXT 80B]:\n" + companion
...
REASONS: {', '.join(reasons)}"""
    ...
    system_prompt = "AEROVULPIS DEEP ANALYSIS ENGINE V3.5. Technical analysis with Entry/SL/TP. Indonesian, max 2000 chars."
...
# =====================================================================
    ...
    with st.expander("SIGNAL ANALYSIS"):
        st.markdown("""
        **Signal Analysis Matrix** displays all 20 technical indicators in a comprehensive grid format.
    ...
    with st.expander("MARKET SESSIONS & NEWS"):
        ...
        **Global Market Session Monitor:**
        - Real-time tracking of Asian (Tokyo), European (London), and American (New York) sessions
        - Visual progress bars showing exact session completion percentage
        - Golden Hour detection for London-New York overlap (19:00-23:00 WIB)
        - SMC-based strategy recommendations adapted to active sessions

        **Market News Aggregator:**
        - Multi-source news from global financial networks
    ...
    with st.expander("SMART ALERT CENTER"):
        st.markdown("""
        **Automated Price Monitoring System** with Telegram notifications.
        ...
        **Features:**
        ...
        - Instant Telegram notification when target price is reached
        - Price formatting auto-adjusts per instrument type
        - Multiple concurrent alerts supported
        """)
    
    with st.expander("CHATBOT AI"):
        st.markdown("""
        **Neural Assistant** powered by the AeroVulpis Engine.

        **Capabilities:**
        - Contextual awareness of selected instrument and live price
        ...
        **Daily Limits by Tier:**
        ...
        - TRIAL: 50/day | WEEKLY: 100/day
        - MONTHLY: 200/day | 6M PRO: 500/day
        - ULTIMATE: Unlimited
        """)
    
    with st.expander("ECONOMIC RADAR"):
        st.markdown("""
        **Global Economic Scanner** monitors high-impact economic events.

        **Features:**
        - Live economic calendar with global event coverage
        - High-impact event detection (3-star events)
        - Currency-specific filtering: USD, EUR, GBP, JPY, AUD, CAD, CHF, NZD

        **Key Events:** Central Bank Rates, NFP, CPI, GDP, PMI, Consumer Confidence
        """)
    
    with st.expander("RISK MANAGEMENT"):
        st.markdown("""
        **Four Pillars Risk Management System:**
        1. **Trading Rules** - Define stop loss and entry parameters
        2. **Position Sizing** - Calculate optimal position size
        3. **Confidence Scores** - Real-time technical strength assessment
        4. **Risk Strategy** - Daily loss limits and profit targets

        **Return Simulator:** R:R ratios, weekly simulation, daily loss/profit limits, full Weekly/Monthly/Yearly projections with balance summary.
        """)
    ...
    with st.expander("LICENSE & TIER SYSTEM"):
        ...
        **Sign In:** Google OAuth 2.0 via secure authentication.
        **License Activation:** Enter key after login to upgrade tier.

        **Tier Levels:**
        - FREE: 5 AI/day, 20 chat/day
        - TRIAL: 10 AI/day, 50 chat/day
        - WEEKLY: 20 AI/day, 100 chat/day
        - MONTHLY: 50 AI/day, 200 chat/day
        - 6M PRO: 100 AI/day, 500 chat/day
        - ULTIMATE: Unlimited

        **Google Auth Setup:**
        1. Google Cloud Console → Redirect URI: `https://bqdugkmmnbxlftuxqtdg.supabase.co/auth/v1/callback`
        2. Supabase Dashboard → Authentication → Providers → Google → Enable + Fill Client ID & Secret
        3. Supabase Dashboard → Authentication → URL Configuration → Site URL: `https://[app].streamlit.app`
        """)
    
    st.info("SETTINGS: Language switching (ID/EN). System cache clear available for troubleshooting.")
