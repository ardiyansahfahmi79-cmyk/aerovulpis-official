...
# ##############################################################################
# CYBER-TECH CSS (LENGKAP - TERMASUK UPGRADE & PAYMENT MODAL)
# ##############################################################################
...
<style>
    ...
    * { font-family: 'Rajdhani', sans-serif; }
    ...
    .close-modal-btn { position: absolute; top: 10px; right: 15px; background: none; border: none; font-size: 20px; color: #557799; cursor: pointer; font-family: 'Orbitron', sans-serif; }
    .close-modal-btn:hover { color: #00d4ff; }
...
def generate_qr_base64(data_string, size=180):
    """Membuat QR code dalam format base64 PNG. Return None jika gagal."""
    try:
        qr = qrcode.QRCode(
            version=1,
...
# ##############################################################################
# SIDEBAR CONTROL CENTER (FIXED - MENU GRATIS TIDAK TERKUNCI)
# ##############################################################################
...
with st.sidebar:
            ...
            if login_submitted and email_input and password_input:
                try:
                    ...
                    if "Email not confirmed" in error_msg:
                        st.error("Please confirm your email address first. Check your inbox and spam folder.")
    ...
    # --- SUDAH LOGIN: TAMPILKAN MENU LENGKAP ---
    ...
    # Hanya blokir jika menu yang dipilih mengandung label "(Premium)" dan user bukan premium
...
# ==============================================================================
# 10. TINGKATKAN LEVEL (UPGRADE PAGE - CYBER-TECH FUTURISTIC & PERBAIKAN)
# ==============================================================================
    ...
    # PERBAIKAN: Gabungkan semua kartu pricing menjadi SATU string HTML
    ...
    for pkg in packages:
        ...
        """
    cards_html += '</div>'
    st.markdown(cards_html, unsafe_allow_html=True)  # Render SEKALI
    ...
    if st.session_state.show_payment_modal:
        ...
        if qr_base64:
            ...
            qr_img_html = '<div class="qr-container"><p style="color:black;padding:20px;">QR Code tidak tersedia</p></div>'

        # Format teks yang akan dikirim ke Telegram
...
Mohon diproses. Berikut bukti pembayaran (lampirkan gambar)."""
        ...
        <div class="payment-modal-overlay" id="payment-modal">
            <div class="payment-modal">
                ...
                <div style="text-align:left;background:rgba(0,20,40,0.5);padding:10px;border-radius:4px;margin:12px 0;font-family:Share Tech Mono;font-size:9px;color:#8899bb;white-space:pre-wrap;">{telegram_text}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        col1, col2, col3 = st.columns([1, 1, 1])
        with col1:
            # Tombol untuk membuka Telegram
            telegram_url = "https://t.me/WartaPrime"
            st.markdown(f'<a href="{telegram_url}" target="_blank"><button style="width:100%;background:#0088cc;color:white;border:none;padding:10px;font-family:Orbitron;letter-spacing:1px;">BAYAR VIA TELEGRAM</button></a>', unsafe_allow_html=True)
        with col2:
            # Tombol untuk menyalin teks ke clipboard (gunakan streamlit komponen atau JavaScript)
            st.markdown(f"""
            <button onclick="navigator.clipboard.writeText(`{telegram_text}`);" style="width:100%;background:rgba(0,212,255,0.2);border:1px solid #00d4ff;color:#00d4ff;padding:10px;font-family:Orbitron;letter-spacing:1px;">COPY TEXT</button>
            """, unsafe_allow_html=True)
        with col3:
