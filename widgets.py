...
def smart_alert_widget():
    ...
    <style>
        .alert-center-container {
            ...
            border-radius: 15px;
            padding: 25px;
            background: rgba(0, 212, 255, 0.05); /* Slightly glowing background */
            box-shadow: 0 0 25px rgba(0, 212, 255, 0.4); /* Stronger glow */
            margin-bottom: 20px;
            position: relative;
            overflow: hidden;
        }
    ...
    with st.container():
        ...
        if st.button("**LOCK TARGET & ACTIVATE SENSOR**", key="activate_sensor_button", help="Click to set the alert and activate the sensor.", type="primary"):
            if price_target > 0 and telegram_chat_id:
                # Placeholder for actual price monitoring logic
                ...
                # Assuming TELEGRAM_BOT_TOKEN is in st.secrets or environment variables
